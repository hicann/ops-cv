...
find_package_handle_standard_args(ascend_protobuf_build_cv
    FOUND_VAR
    ascend_protobuf_build_cv_FOUND
    REQUIRED_VARS
...
if(ascend_protobuf_build_cv_FOUND AND NOT FORCE_REBUILD_CANN_3RD)
    ...
    if(EXISTS "${CANN_3RD_LIB_PATH}/${PROTOBUF_VERSION_PKG}")
        ...
        message(STATUS "[ThirdPartyLib][ascend protobuf] moving ${PROTOBUF_VERSION_PKG} from ${CANN_3RD_LIB_PATH} to ${CANN_3RD_LIB_PATH}/pkg/${PROTOBUF_VERSION_PKG}")
    endif()
    
    # 按优先级顺序检查文件是否存在
    if(EXISTS "${CANN_3RD_LIB_PATH}/pkg/${PROTOBUF_VERSION_PKG}")
        set(REQ_URL "${CANN_3RD_LIB_PATH}/pkg/${PROTOBUF_VERSION_PKG}")
        message(STATUS "[ThirdPartyLib][ascend protobuf] found in file: ${REQ_URL}.")
    elseif(EXISTS "${CANN_3RD_LIB_PATH}/protobuf/${PROTOBUF_ALL_VERSION_NAME}")
        set(REQ_URL "${CANN_3RD_LIB_PATH}/protobuf/${PROTOBUF_ALL_VERSION_NAME}")
        message(STATUS "[ThirdPartyLib][ascend protobuf] found in file: ${REQ_URL}.")
    elseif(EXISTS "${CANN_3RD_LIB_PATH}/protobuf/${PROTOBUF_VERSION_PKG}")
        set(REQ_URL "${CANN_3RD_LIB_PATH}/protobuf/${PROTOBUF_VERSION_PKG}")
        message(STATUS "[ThirdPartyLib][ascend protobuf] found in file: ${REQ_URL}.")
    else()
    ...
    ExternalProject_Add(ascend_protobuf_build_cv
                        URL ${REQ_URL}
    ...
    if(TARGET abseil_build_cv)
        add_dependencies(ascend_protobuf_build_cv abseil_build_cv)
    endif()

    ExternalProject_Get_Property(ascend_protobuf_build_cv SOURCE_DIR)
    ExternalProject_Get_Property(ascend_protobuf_build_cv BINARY_DIR)
    ...
    add_custom_command(
        ...
        DEPENDS ascend_protobuf_build_cv
    )
endif()