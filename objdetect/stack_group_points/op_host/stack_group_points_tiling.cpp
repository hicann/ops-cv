...
void StackGroupPointsTiling::Init()
{
    ...
    OP_CHECK_IF(featuresShape == nullptr,
                OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, featuresShape is null."), return);
    const gert::Shape& fsShape = featuresShape->GetStorageShape();
    const gert::StorageShape* indicesShape = tilingContext->GetInputShape(2);
    OP_CHECK_IF(indicesShape == nullptr, OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, indicesShape is null."),
                return);
    ...
    OP_CHECK_IF(indicesBatchCntShape == nullptr,
    ...
    OP_CHECK_IF(features == nullptr, OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, features is null."),
                return);
    this->featuresSize = features->GetShapeSize();
    auto inputTensor = tilingContext->GetInputTensor(2);
    OP_CHECK_IF(inputTensor == nullptr, OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, inputTensor is null."),
                return);
    ...
    OP_CHECK_IF(zeroInputDesc == nullptr,
                OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, zeroInputDesc is null."), return);
...
ge::graphStatus StackGroupPointsTiling::RunKernelTiling()
{
    ...
    OP_CHECK_IF(currentWorkspace == nullptr,
                OP_LOGE(tilingContext, "StackGroupPoints Tiling Error, currentWorkspace is null."),
...
static ge::graphStatus TilingPrepare4SGP(gert::TilingParseContext* context)
{
    ...
    OP_CHECK_IF((compileInfo->totalCoreNum <= 0 || compileInfo->ubSize <= 0),
                OP_LOGE(context, "StartGroupPoints GetHardwareInfo Failed, vectorCoreNum:%d, ubSize:%ld.",
