...
#include "nn_other.h"
#include "../op_graph/yolox_bounding_box_decode_proto.h"
...
constexpr int INPUT_FILL_VALUE = 2;
constexpr size_t TIME_STRING_BUFFER_SIZE = 64;
...
#define ADD_INPUT_MODE(intputIndex, intputName, intputDtype, inputShape, mode)                                       \
    ...
    ret = GenOnesData(placeholder##intputIndex##_real_shape, tensor_placeholder##intputIndex,                        \
                      placeholder##intputIndex##_desc_real, intputDtype, INPUT_FILL_VALUE);                          \
    if (ret != SUCCESS) {                                                                                            \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    ret = GenOnesData(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                            \
                      placeholder##intputIndex##_desc, intputDtype, INPUT_FILL_VALUE);                              \
    if (ret != SUCCESS) {                                                                                           \
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());                              \
        return FAILED;                                                                                              \
    }                                                                                                               \
...
string GetTime()
{
    time_t timep;
    time(&timep);
    char tmp[TIME_STRING_BUFFER_SIZE];
...
int32_t GenOnesDataFloat32(vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, float value)
{
    ...
    uint32_t byteSizeFloat32 = static_cast<uint32_t>(sizeof(float));
...
int CreateOppInGraph(RunMode mode, DataType inDtype, const std::vector<int64_t>& priorsShape,
                     const std::vector<int64_t>& bboxesShape, std::vector<ge::Tensor>& input,
                     std::vector<Operator>& inputs, std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::YoloxBoundingBoxDecode("add1");
    // priors: (N, 4)，bboxes: (B, N, 4)，两者 N 必须一致
    ADD_INPUT_MODE(1, priors, inDtype, priorsShape, mode);
    ADD_INPUT_MODE(2, bboxes, inDtype, bboxesShape, mode);

    // 输出 decoded_bboxes shape 与 bboxes 一致 (B, N, 4)
    ADD_OUTPUT_MODE(1, decoded_bboxes, inDtype, bboxesShape, mode);

    // ====== 属性设置示例（YoloxBoundingBoxDecode 无属性，此处为模板说明，生成实际算子时替换为真实属性）======
    // 强类型接口 set_attr_{name}(value)：用于 Float/Int/Bool/ListInt/ListFloat/ListBool
    // add1.set_attr_alpha(1.0f);                          // Float
    // add1.set_attr_num(2);                               // Int
    // add1.set_attr_flag(true);                           // Bool
    // add1.set_attr_dims({1, 2, 3});                      // ListInt
    // 弱类型接口 SetAttr(name, value)：仅用于 String/ListString，⚠️ 必须传 const char*，不能传 std::string
    // std::string mode = "NHWC";
    // add1.SetAttr("format", mode.c_str());               // String —— 用 .c_str()
    // add1.SetAttr("format", "NHWC");                     // String —— 字面量直接传
    // =====================================================================================
...
CaseResult RunOneCase(ge::Session* session, uint32_t graph_id, RunMode mode, DataType dtype,
                      const std::vector<int64_t>& priorsShape, const std::vector<int64_t>& bboxesShape,
                      const std::string& case_name)
{
    ...
    Status ret = CreateOppInGraph(mode, dtype, priorsShape, bboxesShape, input, inputs, outputs, graph);
    if (ret != SUCCESS) {
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> global_options = {
        {"ge.exec.deviceId",
         "0"}, // device id 通过`npu-smi info`查询，当前环境 Ascend950PR Health 为 OK，device id 为 0
        {"ge.graphRunMode", "0"},
    ...
    // shape 场景矩阵（YoloxBoundingBoxDecode 输入为 priors(N,4) 与 bboxes(B,N,4)，N 必须一致）
    // 抽象场景模板 {32,4,4,4}/{1}/{64}/{0,8}/{2,2,2,2,2,2,2,2} 中，1维与8维场景对
    // 固定 2D/3D 输入的算子不合法，故裁剪；空tensor 场景取 B=0。
    struct ShapeCombo {
        std::vector<int64_t> priorsShape;
        std::vector<int64_t> bboxesShape;
        std::string name;
    };
    std::vector<ShapeCombo> shape_list = {
        {{16, 4}, {8, 16, 4}, "regular"}, // 常规多维: N=16, B=8
        {{1, 4}, {1, 1, 4}, "narrow"},    // 维度下界: N=1, B=1
        {{2, 4}, {1, 2, 4}, "1d"},        // 最小非平凡: N=2, B=1
        {{2, 4}, {0, 2, 4}, "empty"},     // 空tensor: B=0
    };
    ...
    // N_dtype × N_shape × 1 mode 全矩阵
    // ⚠️ RUN_MODE_D 已裁剪：canndev v1.0 verify（nn_detect_ops.cc YoloxBoundingBoxDecodeVerify）
    // 对动态 shape（-1）执行 check 时报 "input shapes don't match" 失败（上游原型限制，
    // 动态 shape 通路不支持），实测 S 模式全部通过、D 模式全部失败后裁剪，仅保留 S 模式。
    for (const auto& d : dtype_list) {
        for (const auto& s : shape_list) {
            for (auto mode : {RUN_MODE_S}) {
                ...
                CaseResult r = RunOneCase(session, graph_id, mode, d.dt, s.priorsShape, s.bboxesShape, case_name);
                results.push_back(r);
                graph_id++;
            }
        }
