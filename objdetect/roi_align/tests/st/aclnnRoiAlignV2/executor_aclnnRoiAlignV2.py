...
@register("pyaclnn_aclnn_roi_align_v2")
class AsceneRoiAlignV2Api(BaseApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        ...
        for i in range(len(input_data.kwargs["boxes"])):
            input_data.kwargs["boxes"][i][0] = random.uniform(0, int(input_data.kwargs["self"].shape[0]-0.1))
            input_data.kwargs["boxes"][i][1] = random.uniform(0, input_data.kwargs["self"].shape[3] / input_data.kwargs["spatialScale"])
            input_data.kwargs["boxes"][i][2] = random.uniform(0, input_data.kwargs["self"].shape[2] / input_data.kwargs["spatialScale"])
            input_data.kwargs["boxes"][i][3] = random.uniform(float(input_data.kwargs["boxes"][i][1]+0.01), input_data.kwargs["self"].shape[3] / input_data.kwargs["spatialScale"])
            input_data.kwargs["boxes"][i][4] = random.uniform(float(input_data.kwargs["boxes"][i][2]+0.01), input_data.kwargs["self"].shape[2] / input_data.kwargs["spatialScale"])
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        def roi_align_golden(feature_map, rois, pooled_height, pooled_width, spatial_scale, sample_num, roi_end_mode):
        ...
        if self.device == "gpu":
            ...
            output_size = [input_data.kwargs["pooledHeight"], input_data.kwargs["pooledWidth"]]
            RoIAlign = torchvision.ops.RoIAlign(output_size=output_size, sampling_ratio=input_data.kwargs["samplingRatio"],
                                                spatial_scale=input_data.kwargs["spatialScale"], aligned=input_data.kwargs["aligned"])
            output = RoIAlign(input_data.kwargs["self"].to(device), input_data.kwargs["boxes"].to(device))
            return output
            ...
            if input_data.kwargs["aligned"]:
                roi_end_mode = 2
            else:
                roi_end_mode = 0
            res = torch_npu.npu_roi_align(input=input_data.kwargs["self"], rois=input_data.kwargs["boxes"],
                                          spatial_scale=input_data.kwargs["spatialScale"],
                                          pooled_height=input_data.kwargs["pooledHeight"],
                                          pooled_width=input_data.kwargs["pooledWidth"],
            ...
            RoIAlign = torchvision.ops.RoIAlign(output_size=output_size, sampling_ratio=input_data.kwargs["samplingRatio"],
                                                spatial_scale=input_data.kwargs["spatialScale"], aligned=input_data.kwargs["aligned"])
            output = RoIAlign(input_data.kwargs["self"], input_data.kwargs["boxes"])
            return output
 
@register("pyaclnn_aclnn_roi_align_v2_aclnnbase")
class AsceneRoiAlignV2Api(AclnnBaseApi):
        
    def init_by_input_data(self, input_data: InputDataset):
        input_args, output_packages= super().init_by_input_data(input_data)
        input_args[4] = ctypes.c_float(input_args[4].value)
    ...
    #     return "aclnnStatus aclnnRoiAlignV2GetWorkspaceSize(const aclTensor* self, const aclTensor* boxes, int64_t pooledHeight, int64_t pooledWidth, float spatialScale, int64_t samplingRatio, bool aligned, aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)"