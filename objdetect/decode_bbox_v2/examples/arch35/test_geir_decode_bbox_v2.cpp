...
#include "../../op_graph/decode_bbox_v2_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    auto add1 = op::DecodeBboxV2("decode_bbox_v2_1");

    std::vector<int64_t> boxShape = {2, 4};
    ADD_INPUT(1, boxes, inDtype, boxShape);

    ADD_INPUT(2, anchors, inDtype, boxShape);

    std::vector<float> scalesVec = {1.0f, 1.0f, 1.0f, 1.0f};
    add1.SetAttr("scales", scalesVec);

    add1.SetAttr("decode_clip", 0.0f);

    add1.SetAttr("reversed_box", false);
