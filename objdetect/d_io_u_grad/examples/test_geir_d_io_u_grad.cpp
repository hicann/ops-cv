...
#include "nn_other.h"
#include "../op_graph/d_io_u_grad_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::DIoUGrad("add1");
    std::vector<int64_t> dyShape = {8};
    std::vector<int64_t> boxesShape = {4, 8};

    ADD_INPUT(1, dy, inDtype, dyShape);
    ADD_INPUT(2, bboxes, inDtype, boxesShape);
    ADD_INPUT(3, gtboxes, inDtype, boxesShape);

    ADD_OUTPUT(1, dbboxes, inDtype, boxesShape);
    ADD_OUTPUT(2, dgtboxes, inDtype, boxesShape);

    add1.set_attr_is_cross(false);
