                     ...
                     std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::RoiAlignRotatedGrad("add1");
    std::vector<int64_t> xGradShape = {2, 2, 2, 6};
    std::vector<int64_t> roisShape = {6, 2};
    std::vector<int64_t> yGradShape = {5, 1, 1, 6};

    ADD_INPUT(1, x_grad, inDtype, xGradShape);
    ADD_INPUT(2, rois, inDtype, roisShape);
    ADD_OUTPUT(1, y_grad, inDtype, yGradShape);
    std::vector<int64_t> y_grad_shape = {5,1,1,6};
    int64_t pooled_h = 2;
    int64_t pooled_w = 2;
    float spatial_scale = 0.0625;
    int64_t sampling_ratio = 1;
    int64_t aligned = true;
    int64_t clockwise = true;

    add1.set_attr_y_grad_shape(y_grad_shape);
    add1.set_attr_pooled_h(pooled_h);
    add1.set_attr_pooled_w(pooled_w);
    add1.set_attr_spatial_scale(spatial_scale);
    add1.set_attr_sampling_ratio(sampling_ratio);
    add1.set_attr_aligned(aligned);
    add1.set_attr_clockwise(clockwise);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < output_num; i++)
    {
        ...
        LOG_PRINT("th output %d, output shape size =: %ld\n", i, output_shape);
