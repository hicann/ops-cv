...
#include "../../op_graph/bounding_box_decode_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    auto add1 = op::BoundingBoxDecode("bounding_box_decode_1");

    std::vector<int64_t> boxShape = {2, 4};
    ADD_INPUT(1, rois, inDtype, boxShape);

    ADD_INPUT(2, deltas, inDtype, boxShape);
    ...
    std::vector<int64_t> maxShapeVec = {768, 1280};
    add1.SetAttr("max_shape", maxShapeVec);

    add1.SetAttr("wh_ratio_clip", 0.016f);

    ADD_OUTPUT(1, bboxes, inDtype, boxShape);
