...
#include "nn_other.h"
#include "../op_graph/g_io_u_grad_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    giou_grad1.set_input_##intputName(placeholder##intputIndex);                                                    \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    giou_grad1.update_output_desc_##outputName(outputName##outputIndex##_desc);
                    ...
                    int value)
{
    ...
    uint32_t data_type_size = GetDataTypeSize(data_type);
    uint32_t data_len = size * data_type_size;
    ...
    if (data_type == ge::DT_FLOAT) {
        ...
        for (size_t i = 0; i < size; ++i) {
            memcpy(pData + i * data_type_size, &fval, data_type_size);
        }
    } else {
        for (size_t i = 0; i < size; ++i) {
            memcpy(pData + i * data_type_size, &value, data_type_size);
        }
    }
...
int32_t WriteDataToFile(string bin_file, uint64_t data_size, uint8_t* inputData)
{
    ...
    if (fp == nullptr) {
        printf("WriteDataToFile: failed to open file %s\n", bin_file.c_str());
        return FAILED;
    }
...
{
    Status ret = SUCCESS;
    // GiouGrad: 3 inputs (dy, bboxes, gtboxes) -> 2 outputs (dbboxes, dgtboxes)
    auto giou_grad1 = op::GIoUGrad("giou_grad1");
    int64_t N = 32;
    std::vector<int64_t> dyShape = {N};
    std::vector<int64_t> boxShape = {4, N};
    ADD_INPUT(1, dy, inDtype, dyShape);
    ADD_INPUT(2, bboxes, inDtype, boxShape);
    ADD_INPUT(3, gtboxes, inDtype, boxShape);

    ADD_OUTPUT(1, dbboxes, inDtype, boxShape);
    ADD_OUTPUT(2, dgtboxes, inDtype, boxShape);

    giou_grad1.set_attr_is_cross(false);

    outputs.push_back(giou_grad1);
    return SUCCESS;
}
