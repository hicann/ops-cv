...
#include "nn_other.h"
#include "../op_graph/anchor_response_flags_proto.h"
...
{
    Status ret = SUCCESS;
    // Custom code: add anchor_response_flags operator to graph
    auto add1 = op::AnchorResponseFlags("add1");
    add1.set_attr_featmap_size({60, 60});
    add1.set_attr_strides({2, 2});
    add1.set_attr_num_base_anchors(9);
    std::vector<int64_t> xShape = {100, 4};
    ADD_INPUT(1, gt_bboxes, inDtype, xShape);

    // Output shape: feat_h * feat_w * num_base_anchors = 60 * 60 * 9 = 32400
    std::vector<int64_t> outShape = {32400};
    ADD_OUTPUT(1, flags, ge::DT_UINT8, outShape);
