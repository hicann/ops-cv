...
class KernelIouV2Align : public KernelIouV2AlignBase<inType, KernelIouV2Align> {
public:
    __aicore__ inline void Init(GM_ADDR bboxes, GM_ADDR gtboxes, GM_ADDR overlap, const IouV2TilingData* tilingData)
    {
        ...
        KernelIouV2AlignBase<inType, KernelIouV2Align>::Init(bboxes, gtboxes, overlap, tilingData, (8 * tileLen) * 4);
    }
    ...
    __aicore__ inline void Compute()
    {
        ...
        Sub(tmpBuffer, box1Local[this->doubleTileLen], box1Local, this->doubleTileLen);
        Sub(tmpBuffer[this->doubleTileLen], box2Local[this->doubleTileLen], box2Local, this->doubleTileLen);
        Adds(tmpBuffer, tmpBuffer, this->eps, this->quadTileLen);
        Mul(tmpBuffer, tmpBuffer, tmpBuffer[this->tileLen], this->tileLen);
        Mul(tmpBuffer[this->doubleTileLen], tmpBuffer[this->doubleTileLen], tmpBuffer[this->tripTileLen],
            this->tileLen);

        LocalTensor<float> outLocal = this->outQue.template AllocTensor<float>();
        Add(outLocal, tmpBuffer, tmpBuffer[this->doubleTileLen], this->tileLen);

        Max(tmpBuffer, box1Local, box2Local, this->doubleTileLen);
        Min(tmpBuffer[this->doubleTileLen], box1Local[this->doubleTileLen], box2Local[this->doubleTileLen],
            this->doubleTileLen);
        ...
        Mul(tmpBuffer, tmpBuffer, tmpBuffer[this->tileLen], this->tileLen);
        Sub(outLocal, outLocal, tmpBuffer, this->tileLen);
