...
static aclnnStatus CheckParams(const aclTensor* bBoxes, const aclTensor* gtBoxes, aclTensor* overlap, const char* mode,
                               float eps, bool aligned)
{
    // 1. 检查参数是否为空指针
...
aclnnStatus aclnnIouGetWorkspaceSize(const aclTensor* bBoxes, const aclTensor* gtBoxes, const char* mode, float eps,
                                     bool aligned, aclTensor* overlap, uint64_t* workspaceSize,
