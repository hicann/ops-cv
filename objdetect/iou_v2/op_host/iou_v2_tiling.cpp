...
static ge::graphStatus Tiling4IouV2(gert::TilingContext* context)
{
    ...
    if (!aligned) {
        ...
        totalDataSize += (dataSize == FLOAT_SIZE ?
                              0 :
                              gtBoxLength * validSubLen * 2); // f16/bf16需要额外分配2块buffer用于cast
        tempLoop = CeilDiv(totalDataSize, MIN_SIZE_PER_CORE);
        maxLength =
            64; // 64是由于Max和Min的mask不能超过64，否则引入精度问题；正常非对齐的最大长度应为72，牺牲部分长度换精度
...
ge::graphStatus TilingParse4IouV2([[maybe_unused]] gert::TilingParseContext* context) { return ge::GRAPH_SUCCESS; }

IMPL_OP_OPTILING(IouV2).Tiling(Tiling4IouV2).TilingParse<IouV2CompileInfo>(TilingParse4IouV2);
} // namespace optiling
