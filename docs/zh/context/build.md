...
1. **配置环境变量**
   ...
   参考[环境准备](./quick_install.md#环境准备三选一)完成环境变量配置。
   ```bash
   # 默认路径安装，以root用户为例
...
| --jit           | 可选     | 静态图场景下，编译`cann-${soc_name}-ops-cv_${cann_version}_linux-${arch}.run`整包时不需要编译算子二进制文件（图的运行态会在线编译），可以配置该选项，以提升编译速度。 |
