...
|          | Bool/Float/Int... | 设置算子属性数据类型为“Bool/Float/Int...”。                    |
...
```mermaid
flowchart TD
    A["算子的输入、输出以及属性信息<br/>通过TilingContext* context获取"]
    B["Tiling"]
    C["TilingData、BlockDim、TilingKey、workspace size...<br/>设置到TilingContext* context中"]
...
|------|------|
| 生成代码 | `build/autogen/` |
| 动态库 | `${ASCEND_HOME_PATH}/cann-{version}/opp/vendors/custom_cv/op_api/lib/` |
| 头文件 | `${ASCEND_HOME_PATH}/cann-{version}/opp/vendors/custom_cv/op_api/include/` |
