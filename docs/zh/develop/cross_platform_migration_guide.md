...
这套信号量的本质是保证多线程/多流水阶段间一致的同步序列，防止因资源未就绪或依赖没完成而发生数据竞争或死锁等硬件异常。CrossCoreSetFlag与CrossCoreWaitFlag接口说明可参考官方文档：[《Ascend C API》](https://hiascend.com/document/redirect/CannCommunityAscendCApi)。
