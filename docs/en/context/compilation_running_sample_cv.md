...
Assume that the development environment and operating environment are deployed on the same server (equipped with the AI processor). In this scenario, code development and code running are performed on the same machine. The following uses the GridSample operator as an example. The calling logic, process, and compilation script of other operators are similar to those of the GridSample operator. You should modify the API calling script (**\*.cpp**) and compilation script (**CMakeLists**) as required.

- **Sample code**

   The GridSample operator is used to provide an input tensor and a grid, and fill the pixel value of a location in the input to the location specified by the grid based on the coordinates of the location provided in the grid to obtain the final output. You can obtain the sample code from "Example" in [aclnnGridSampler2D](../../../image/grid_sample/docs/aclnnGridSampler2D_en.md) and name the code file **test\_grid\_sampler2\_d.cpp**.
...
## Compilation and Running
  ...
  3. Compile and run the script.
      ...
      - Go to the **bin** folder and run the executable file **opapi\_test**.
        ...
        The following uses the running result of the GridSample operator as an example:
