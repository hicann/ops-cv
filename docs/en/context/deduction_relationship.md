...
For the data types supported by aclTensor, refer to [Data Type](data_type.md). Some types satisfy the following derivation rules. The derivation principle is similar to PyTorch's [Type Promotion](https://pytorch.org/docs/stable/tensor_attributes.html#type-promotion-doc).
...
>- The cross mark (×) in the table indicates that these two types cannot perform derivation calculation.
...
- When calling the aclnnAdd interface, if the data types of the input parameters are inconsistent, one is float16 and one is float32, then the API internally converts the float16 data type to the float32 data type and then performs the calculation.
- When calling the aclnnAdd interface, if the data types of the input parameters are inconsistent, one is float32 and one is bool, then the API internally converts the bool data type to the float32 data type and then performs the calculation.
