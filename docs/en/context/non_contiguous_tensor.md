...
Currently, the input aclTensor of most operator APIs supports "**non-contiguous Tensor**", that is, a Tensor can be represented by \(shape, strides, offset\).

Note: Creating an aclTensor can be implemented through "Public Interface > aclCreateTensor" in [Operator Library Interface](https://hiascend.com/document/redirect/CannCommunityOplist).

## Example 1

For example, there is a Tensor with shape=\(6, 5\), strides=\(10, 1\), offset=22. Its memory layout is as follows:
...
That is, the Tensor is laid out in the dark positions shown in the figure above. This complete Tensor is non-contiguous in memory layout. strides describe the interval between two adjacent elements in the Tensor dimension. If the stride in dimension 1 is 1, the dimension is contiguous; if the stride in dimension 0 is 10, then adjacent elements are separated by 10 elements, that is, non-contiguous. offset represents the offset of the first element of this Tensor relative to addr.

## Example 2

For example, there is a Tensor with shape=\(4, 3\), strides=\(20, 2\), offset=22. Its memory layout is as follows:
...
That is, the Tensor is laid out in the dark positions shown in the figure above. This complete Tensor is non-contiguous in memory layout. strides describe the interval between two adjacent elements in the Tensor dimension. If the stride in dimension 1 is 2, the elements in that dimension are separated by 1 element; if the stride in dimension 0 is 20, then adjacent elements are separated by 20 elements, that is, non-contiguous. offset represents the offset of the first element of this Tensor relative to addr.
