...
static void ref_ciou(const std::vector<float>& bb, const std::vector<float>& gb, int N, bool trans, int modeId,
                     bool atanSubFlag, std::vector<float>& overlap, std::vector<float>& atanSub)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (trans) {
            ...
            gx2 = b2;
            gy2 = b3;
            bw = std::max(0.0f, bx2 - bx1);
            bh = std::max(0.0f, by2 - by1);
            gw = std::max(0.0f, gx2 - gx1);
            gh = std::max(0.0f, gy2 - gy1);
        }
...
static void make_no_overlap_boxes(int N, float scale, std::vector<float>& bb, std::vector<float>& gb)
{
    ...
    for (int i = 0; i < N; ++i) {
        float x1 = ua(ra), x2 = ua(ra);
        if (x2 < x1)
            std::swap(x1, x2);
        x2 += 0.5f;
        float y1 = ua(ra), y2 = ua(ra);
        if (y2 < y1)
            std::swap(y1, y2);
        y2 += 0.5f;
        bb[0 * N + i] = x1;
        ...
        bb[2 * N + i] = x2;
        bb[3 * N + i] = y2;
        float gx1 = ub(rb), gx2 = ub(rb);
        if (gx2 < gx1)
            std::swap(gx1, gx2);
        gx2 += 0.5f;
        float gy1 = ub(rb), gy2 = ub(rb);
        if (gy2 < gy1)
            std::swap(gy1, gy2);
        gy2 += 0.5f;
        gb[0 * N + i] = gx1;
...
static void make_boxes(const Case& c, std::vector<float>& bb, std::vector<float>& gb)
{
    ...
    if (d == "full_overlap") {
        if (c.trans) {
            fill_rand_xywh(bb, N, scale, 1);
            gb = bb;
        } else {
            fill_rand_xyxy(bb, N, scale, 1);
            gb = bb;
        }
        ...
        fill_rand_xywh(bb, N, scale, 10);
        fill_rand_xywh(gb, N, scale, 11);
    } else {
        fill_rand_xyxy(bb, N, scale, 12);
        fill_rand_xyxy(gb, N, scale, 13);
    }
}
                               ...
                               void** bdev, void** gdev, void** odev, void** adev)
{
    ...
    if (c.dt == ACL_FLOAT) {
        ...
        rc = CreateAclTensor(bb_f, boxShape, bdev, c.dt, bboxes);
        if (rc)
            return rc;
        rc = CreateAclTensor(gb_f, boxShape, gdev, c.dt, gtboxes);
        if (rc)
            return rc;
        rc = CreateAclTensor(oh, outShape, odev, c.dt, overlap);
        if (rc)
            return rc;
        rc = CreateAclTensor(ah, outShape, adev, c.dt, atanSub);
        if (rc)
            return rc;
    } else if (c.dt == ACL_FLOAT16) {
        std::vector<uint16_t> bh(4 * N), gh(4 * N), oh(N, 0), ah(N, 0);
        for (int i = 0; i < 4 * N; ++i) {
            bh[i] = fp32_to_fp16(bb_f[i]);
            gh[i] = fp32_to_fp16(gb_f[i]);
        }
        rc = CreateAclTensor(bh, boxShape, bdev, c.dt, bboxes);
        if (rc)
            return rc;
        rc = CreateAclTensor(gh, boxShape, gdev, c.dt, gtboxes);
        if (rc)
            return rc;
        rc = CreateAclTensor(oh, outShape, odev, c.dt, overlap);
        if (rc)
...
static int execute_ciou(aclrtStream stream, const Case& c, aclTensor* bboxes, aclTensor* gtboxes, aclTensor* overlap,
                        aclTensor* atanSub, void*& wsAddr)
{
    char modeBuf[8];
    std::snprintf(modeBuf, sizeof modeBuf, "%s", c.mode);
    uint64_t wsSize = 0;
    aclOpExecutor* executor = nullptr;
    int rc = aclnnCIoUGetWorkspaceSize(bboxes, gtboxes, c.trans, false, modeBuf, c.atanSubFlag, overlap, atanSub,
                                       &wsSize, &executor);
    if (rc != ACL_SUCCESS) {
        LOG_PRINT("[FAIL %s] GetWS=%d\n", c.name, rc);
        return rc;
    }
    if (wsSize > 0) {
...
static void copy_case_outputs(const Case& c, int N, void* odev, void* adev, std::vector<float>& overlap_dev,
                              std::vector<float>& atan_dev)
{
    ...
    if (c.dt == ACL_FLOAT) {
        ...
        for (int i = 0; i < N; ++i) {
            overlap_dev[i] = fp16_to_fp32(oh[i]);
            atan_dev[i] = fp16_to_fp32(ah[i]);
        }
    }
}
...
static int run_case(aclrtStream stream, const Case& c, float& max_err_overlap, float& max_err_atan)
{
    ...
    int rc = create_case_tensors(c, bb_f, gb_f, boxShape, outShape, &bboxes, &gtboxes, &overlap, &atanSub, &bdev, &gdev,
                                 &odev, &adev);
    if (rc)
        return rc;
