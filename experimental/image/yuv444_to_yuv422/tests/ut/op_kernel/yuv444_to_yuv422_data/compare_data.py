...
def compare_data(golden_file_lists, output_file_lists, d_type):
    # For yuv444_to_yuv422, output is uint8 regardless of input dtype
    np_dtype = np.uint8
    ...
    for gold, out in zip(golden_file_lists, output_file_lists):
        ...
        # For uint8, allow max absolute error of 1
        diff = np.abs(tmp_out.astype(np.int32) - tmp_gold.astype(np.int32))
        max_diff = np.max(diff) if len(diff) > 0 else 0
        diff_res = diff <= 1
        diff_idx = np.where(diff_res != True)[0]
        if len(diff_idx) == 0:
            print(f"PASSED! max_diff={max_diff}")
        else:
            print(f"FAILED! max_diff={max_diff}")
            for idx in diff_idx[:5]:
                print(f"index: {idx}, output: {tmp_out[idx]}, golden: {tmp_gold[idx]}, diff: {diff[idx]}")
            data_same = False
    return data_same
