...
int main()
{
    ...
    std::vector<uint8_t> selfHostData = {0, 1, 2, 3, 254, 255, 7, 8};
    std::vector<uint8_t> outHostData(64, 0);
    ...
    ret = aclnnUpsampleNearest3dGetWorkspaceSize(self, outputSize, scalesD, scalesH, scalesW, out, &workspaceSize,
                                                 &executor);
    ...
    for (int64_t d = 0; d < 4; ++d) {
        for (int64_t h = 0; h < 4; ++h) {
            for (int64_t w = 0; w < 4; ++w) {
                const int64_t outIndex = d * 16 + h * 4 + w;
                const int64_t inIndex = (d / 2) * 4 + (h / 2) * 2 + (w / 2);
                exact = exact && (resultData[outIndex] == selfHostData[inIndex]);
            }
        }
    }
    LOG_PRINT("uint8_boundary_values_preserved: %s\n", (resultData[32] == 254 && resultData[34] == 255) ? "YES" : "NO");
    LOG_PRINT("exact_match_to_cpu_reference: %s\n", exact ? "YES" : "NO");
    CHECK_RET(exact, LOG_PRINT("RESULT: FAIL\n"); return ACL_ERROR_INTERNAL_ERROR);
