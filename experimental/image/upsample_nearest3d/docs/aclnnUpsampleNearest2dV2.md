...
[📄 查看源码](https://gitcode.com/cann/ops-cv/tree/master/experimental/image/upsample_nearest3d)
...
- 接口功能：对由多个输入通道组成的输入信号应用最近邻插值算法进行上采样。如果输入shape为(N, C, H, W)，则输出shape为(N, C, outputSize[0], outputSize[1])。本接口相较于[aclnnUpsampleNearest2d](../../../../image/resize_nearest_neighbor_v2/docs/aclnnUpsampleNearest2d.md)，增加入参scalesH、scalesW，请根据实际情况选择合适的接口。
...
每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnUpsampleNearest2dV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnUpsampleNearest2dV2”接口执行计算。
...
int main()
{
    ...
    ret = aclnnUpsampleNearest2dV2GetWorkspaceSize(self, outputSizeArray, scalesH, scalesW, out, &workspaceSize,
                                                   &executor);
