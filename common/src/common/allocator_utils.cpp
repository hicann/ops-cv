...
uint32_t CpuKernelAllocatorUtils::ParamCheck(
  ...
  KERNEL_CHECK_NULLPTR(output_result_tensor, KERNEL_STATUS_PARAM_INVALID,
                       "output_result_tensor nullptr");
...
uint32_t CpuKernelAllocatorUtils::UpdateOutputDataTensor(
  ...
  uint32_t check_ret = ParamCheck(dims, &data_ptr, output_result_tensor);
