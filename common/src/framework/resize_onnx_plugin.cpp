...
#include "resize_nearest_neighbor_v2_proto.h"
...
static Status BuildNearestResize(
    const std::string& ori_name, Operator& resize_x, Operator& sizes, Operator& resize_roi, Operator& resize_scales,
    const std::string& input_roi, const std::string& input_scales, int input_size, bool align_corners,
    bool half_pixel_centers, std::vector<Operator>& inputs,
...
static Status BuildInterpolatingResize(
    const std::string& ori_name, Operator& resize_x, Operator& sizes, Operator& resize_roi, Operator& resize_scales,
    Operator& data1, Operator& data2, const std::string& input_roi, const std::string& input_scales,
    const std::string& coordinate_transformation_mode_value, const std::string& mode_value,
    std::vector<Operator>& inputs, std::vector<std::pair<Operator, std::vector<size_t>>>& output_indexs)
{
...
static Status ParseResizeModeAttrs(
    const Operator& op, std::string& coordinate_transformation_mode_value, bool& half_pixel_centers,
    bool& align_corners, std::string& mode_value)
{
...
static Status BuildResizeSizesFromInputs(
    const std::string& ori_name, int input_size, ge::Operator& resize_x, ge::Operator& resize_scales,
    ge::Operator& resize_sizes, ge::Operator& sizes)
{
...
static Status ParseOpToGraphResize(const Operator& op, Graph& graph)
{
    ...
    if (mode_value == "nearest") {
        auto status = BuildNearestResize(
            ori_name, resize_x, sizes, resize_roi, resize_scales, input_roi, input_scales, input_size, align_corners,
            half_pixel_centers, inputs, output_indexs);
        if (status != SUCCESS)
            return status;
    } else if (mode_value == "linear" || mode_value == "cubic") {
        auto status = BuildInterpolatingResize(
            ori_name, resize_x, sizes, resize_roi, resize_scales, data1, data2, input_roi, input_scales,
            coordinate_transformation_mode_value, mode_value, inputs, output_indexs);
        if (status != SUCCESS)
            return status;
    } else {
...
static Status BuildNearestResizeV10(
    const std::string& ori_name, Operator& resize_x, Operator& sizes, std::vector<Operator>& inputs,
    std::vector<std::pair<Operator, std::vector<size_t>>>& output_indexs)
{
    auto size = CreateSliceForResize(ori_name, sizes);
    inputs.push_back(size);
...
static Status BuildLinearResizeV10(
    const std::string& ori_name, Operator& resize_x, Operator& sizes, const std::string& mode_value,
    std::vector<Operator>& inputs, std::vector<std::pair<Operator, std::vector<size_t>>>& output_indexs)
{
...
REGISTER_CUSTOM_OP("PartitionedCall")
    .FrameworkType(ONNX)
    .OriginOpType(
        {ge::AscendString("ai.onnx::11::Resize"), ge::AscendString("ai.onnx::12::Resize"),
