...
counter=1
find "$RUN_PACKAGE_SAVE_AB_PATH" -name "cann-ops-cv-custom_operator_group*.run" -type f | while read -r runfile; do
...
for runfile in kernel_*.run; do
    ...
    full_path=$(find "$PARENT_DIR" -maxdepth 1 -type d -name "custom_*cv" | head -n 1)
    if [ -n "$full_path" ]; then
    # 4. 提取目录名 (这就是你要的 custom_operator_group_3_cv)
    ...
    if $first_run; then
        ...
        dest_conf_ascend_first="$HOST_EXTRACT_DIR/${PKG_NAME}/built-in/op_impl/ai_core/tbe/kernel/config/$SOC/ops_cv"
        ...
        dest_conf_ascend="$HOST_EXTRACT_DIR/${PKG_NAME}/built-in/op_impl/ai_core/tbe/kernel/config/$SOC/ops_cv"
...
filelist_src_path="$HOST_EXTRACT_DIR/share/info/ops_cv/script/filelist.csv"
