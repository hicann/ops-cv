...
Decompress multiple cann-ops-cv-static tar.gz packages and merge them.
...
merge_static_lib() {
    local pkg_dir="$1"
    local base_dir="$2"
    local other_lib="$pkg_dir/lib64/libcann_cv_static.a"
...
merge_packages() {
    ...
    PKG_DIRS=()
    for dir in "$TMPDIR"/cann-ops-cv-static-*/; do
        [[ -d "$dir" ]] && PKG_DIRS+=("$dir")
    done

    if [[ ${#PKG_DIRS[@]} -eq 0 ]]; then
        log_error "no cann-ops-cv-static-* directories found after extraction"
        exit 1
    fi

    local VERSION
    VERSION=$(grep 'set_cann_package' "$(dirname "$0")/../../version.cmake" | sed 's/.*set_cann_package([^ ]* [^ ]* "\([^"]*\)".*/\1/')
    local base_name="cann-${SOC}-ops-cv-static-${VERSION}_linux-${ARCH_INFO}"
