...
for log_dir_path in log_dirs:
    ...
    for log_file in log_dir_path.iterdir():
        ...
        try:
            ...
            logging.error(f"read file fail: {log_file}, {e}")
            continue
        if len(lines) < 1:
            continue
        
        # 约定：第一行一定以时间戳+Build started开始，最后一行以时间戳+exe_time结束 
        # 参照ops-cv\scripts\kernel\binary_script\build_binary_op_exe_task.sh
...
for soc_name, op_dict in all_stats.items():
    ...
    logging.info(f"The {soc_name} statistical results have been saved to {csv_path}")
