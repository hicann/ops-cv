...
usage() {
    ...
    echo "    bash ops_st_test.sh --soc_version=ascend910b --ops=resize_bilinear_v2,roi_align"
...
get_changed_ops() {
    ...
    local tmp_file="${build_path}/tmp/git_diff_filelist.txt"
    mkdir -p "${build_path}/tmp"
    echo "${changed_files}" > "${tmp_file}"
    local result=$(parse_ops_from_filelist "${tmp_file}")
    rm -f "${tmp_file}"
...
download_ops_test_kit() {
    ...
    if [[ ! -d "${ttk_path}" ]]; then
        ...
        print_msg "ASCEND_HOME_PATH: ${ASCEND_HOME_PATH}"
        (cd "${build_path}" && cmake -DDOWNLOAD_OPS_TEST_KIT=ON -DASCEND_INSTALL_PATH="${ASCEND_HOME_PATH}" "${framework_path}") || {
