...
OPP_COMPATIBILITY_CEHCK_ERR_DES="OppCv compatibility check error."
PERM_DENIED="0x0093"
...
startlog() {
  echo "[OpsCv] [$(getdate)] [INFO]: Start Time: $(getdate)"
}

exitlog() {
  echo "[OpsCv] [$(getdate)] [INFO]: End Time: $(getdate)"
}
...
check_docker_path() {
  docker_path="$1"
  if [ "${docker_path}" != "/"* ]; then
    echo "[OpsCv] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --docker-root\
 must with absolute path that which is start with root directory /. Such as --docker-root=/${docker_path}"
    exitlog
    exit 1
  fi
  if [ ! -d "${docker_path}" ]; then
    echo "[OpsCv] [ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${docker_path} not exist, please create this directory."
    exitlog
    exit 1
  fi
}
...
judgment_path() {
  ...
  if [ $? -ne 0 ]; then
    echo "[OpsCv][ERROR]: The opp install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
    exitlog
    exit 1
  fi
}
...
check_install_path() {
  ...
  if [ "x${TARGET_INSTALL_PATH}" = "x" ]; then
    echo "[OpsCv] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --install-path\
 ...
 not support that the install path contains space character."
  ...
  if [ "x${prefix}" = "x" ]; then
    ...
    if [ x"${prefix}" = "x" ]; then
      ...
      echo "[OpsCv] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES: Run package path is invalid: $RUN_PATH"
      exitlog
      exit 1
    fi
  fi
  ...
  while true; do
    ...
    case "$1" in
      ...
      --install-for-all)
        ...
        ;;
      -*)
        echo "[OpsCv] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Unsupported parameters [$1],\
...
check_opts() {
  if [ "${CONFLICT_CMD_NUMS}" != 1 ]; then
    echo "[OpsCv] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:\
...
check_pre_install() {
  ...
  if [ "${IS_UPGRADE}" = "y" ]; then
    if [ ! -e "${INSTALL_INFO_FILE}" ]; then
      logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${TARGET_INSTALL_PATH} not install OpsCV, upgrade failed."
      exitlog
      exit 1
    fi
