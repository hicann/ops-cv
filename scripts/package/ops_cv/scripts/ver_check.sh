...
check_opp_version_file() {
  ...
  # ops_cv/version.info -> ops_cv
    ...
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [ops_cv] version.info not exists."
  fi
  return
}
...
show_relation() {
  ...
  if [ "$relation_situation" = "SUCC" ]; then
    logandprint "[INFO]: Relationship of ops_cv with ${req_pkg_name_val} in path ${req_pkg_path} check successfully"
    return 0
  else
    logandprint "[WARNING]: Relationship of ops_cv with ${req_pkg_name_val} in path ${req_pkg_path} check failed. \
