...
install_es_whl() {
  ...
  if [ -d "${TARGET_VERSION_DIR}/ops_cv" ]; then
    rm -rf "${TARGET_VERSION_DIR}/ops_cv"
  fi 
}

add_init_py() {
  local opp_builtin_mod=""
  local built_in_impl_path=${TARGET_OPP_BUILT_IN}/op_impl/ai_core/tbe/impl/ops_cv
