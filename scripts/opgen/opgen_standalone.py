...
class OpGenerator:
    ...
    def _copy_template(self):
        ...
        if not os.path.exists(self.template_dir):
            raise FileNotFoundError(f"找不到模板目录 '{self.template_dir}'。请确保 'template/add_example' 目录存在。")
...
def register_parser(subparsers):
    ...
    parser_opgen.add_argument('--op_type', '-t', required=True, help='算子分类，例如 image')
    parser_opgen.add_argument('--op_name', '-n', required=True, help='新算子的名称，例如 grid_sample')
...
def main():
    ...
    parser.add_argument('--op_type', '-t', required=True, help='算子分类，例如 image')
    parser.add_argument('--op_name', '-n', required=True, help='新算子的名称，例如 grid_sample')
