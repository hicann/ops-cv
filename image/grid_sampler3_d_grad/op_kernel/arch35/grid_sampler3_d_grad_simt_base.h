...
template <typename T>
__simt_callee__ __aicore__ __attribute__((always_inline)) inline T UnnormalizeSetGrad(
    T coord, uint32_t size, uint32_t alignCorners, T* gradInValue)
{
    float coord_f = static_cast<float>(coord);
    if (alignCorners == 1) {
        // unnormalize coord from [-1, 1] to [0, size - 1]
        coord_f = (coord_f + 1) / CONST_TWO * (size - 1);
        *gradInValue = static_cast<T>(size - 1) / CONST_TWO;
    } else {
        // unnormalize coord from [-1, 1] to [-0.5, size - 0.5]
        coord_f = ((coord_f + 1) * size - 1) / CONST_TWO;
        *gradInValue = static_cast<T>(size) / CONST_TWO;
    }
    return static_cast<T>(coord_f);
}

template <typename T>
__simt_callee__ __aicore__ __attribute__((always_inline)) inline T ClipCoorDinatesSetGrad(
    T coord, uint32_t clip_limit, float* gradClipValue)
{
...
template <typename T>
__simt_callee__ __aicore__ __attribute__((always_inline)) inline T ReflectCoordinatesSetGrad(
    T coord, int twiceLow, uint32_t twiceHigh, float* gradReflValue)
{
...
template <typename T>
__simt_callee__ __aicore__ __attribute__((always_inline)) inline T ComputeSourceIndexSetGrad(
    T coord, uint32_t size, uint32_t padding, uint32_t alignCorners, T* gradInValue)
{
