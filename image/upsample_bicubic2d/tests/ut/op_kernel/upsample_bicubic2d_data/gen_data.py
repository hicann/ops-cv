...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    y_golden = torch.nn.functional.interpolate(x_tensor, output_size, mode='bicubic')
    tmp_golden = np.array(y_golden).astype(np_type)

    tmp_input.astype(np_type).tofile(f"{d_type}_input_bicubic2d.bin")
    tmp_golden.astype(np_type).tofile(f"{d_type}_golden_bicubic2d.bin")
