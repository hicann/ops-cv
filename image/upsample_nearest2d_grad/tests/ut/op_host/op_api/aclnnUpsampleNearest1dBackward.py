...
def gen_golden(case_info: dict):
  ...
  if input_dtype == "float16":
      self_tensor = torch.from_numpy(input_desc[0]["value"].astype(np.float32))
  else:
      self_tensor = torch.from_numpy(input_desc[0]["value"])
  output_size_desc = input_desc[1]["value"]
  input_size_desc = input_desc[2]["value"]

  output = torch.ops.aten.upsample_nearest1d_backward(self_tensor, output_size_desc, input_size_desc)
  if input_dtype == "float16":
      output = output.half()
  return output.numpy()