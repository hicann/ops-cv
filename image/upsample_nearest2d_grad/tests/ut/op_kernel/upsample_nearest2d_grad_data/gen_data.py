...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    grad_output.astype(np_type).tofile(f"{d_type}_input_upsample_nearest2d_grad.bin")
    grad_input_golden.astype(np_type).tofile(f"{d_type}_golden_upsample_nearest2d_grad.bin")
