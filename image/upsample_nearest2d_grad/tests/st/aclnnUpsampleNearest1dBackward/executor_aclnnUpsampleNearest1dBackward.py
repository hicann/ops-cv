...
@register("upsample_nearest1d_backward")
class FunctionTorchUpsampleNearest1dBackwardApi(FunctionApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        backmarkgradoutput = input_data.kwargs['input'].grad
        return backmarkgradoutput

    def init_by_input_data(self, input_data: InputDataset):
        input_temp = input_data.kwargs['input']
        if self.device == "pyaclnn":
            ...
            #处理L的取值,通过scale_factor和size取值计算outputSize_int的取值，区分是tuple和非tuple
            if 'scale_factor' in input_data.kwargs and not isinstance(input_data.kwargs['scale_factor'], tuple):
                scale_factor_temp = input_data.kwargs['scale_factor']
                input_data.kwargs['scales'] = input_data.kwargs['scale_factor']
                input_data.kwargs['outputSize_int'][0] = int(input_temp.shape[2]*scale_factor_temp)
