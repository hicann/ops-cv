                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::IMGWarpResize("add1");
    // img: [N, 4, C, H, W]
    std::vector<int64_t> imgShape = {1, 4, 3, 4, 4};
    // warp_index: [N, 2, H, W]
    std::vector<int64_t> warpIndexShape = {1, 2, 4, 4};
    // warp_img: [N, C, H, W]
    std::vector<int64_t> outShape = {1, 3, 4, 4};
    ADD_INPUT(1, img, inDtype, imgShape);
    ADD_INPUT(2, warp_index, inDtype, warpIndexShape);

    ADD_OUTPUT(1, warp_img, inDtype, outShape);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        uint64_t data_size = input_shape * GetDataTypeSize(input[i].GetTensorDesc().GetDataType());
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        uint64_t data_size = output_shape * GetDataTypeSize(output[i].GetTensorDesc().GetDataType());
