...
#include "../op_graph/scale_and_translate_proto.h"
...
#define ADD_INPUT_INT(inputIndex, inputName, inputDtype, inputShape)                       \
    vector<int64_t> placeholder##inputIndex##_shape = inputShape;                          \
    auto placeholder##inputIndex = op::Data("placeholder" + inputIndex).set_attr_index(0); \
    TensorDesc placeholder##inputIndex##_desc =                                            \
        TensorDesc(ge::Shape(placeholder##inputIndex##_shape), FORMAT_NHWC, inputDtype);   \
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::ScaleAndTranslate("scale_and_translate");
    std::vector<int64_t> imagesShape = {1, 4, 4, 3};
    std::vector<int64_t> sizeShape = {2};
    std::vector<int64_t> scaleShape = {2};
    std::vector<int64_t> translationShape = {2};

    ADD_INPUT(1, images, DT_FLOAT, imagesShape, 1.0);
    ADD_INPUT_INT(2, size, DT_INT32, sizeShape);
    ADD_INPUT(3, scale, DT_FLOAT, scaleShape, 1.0);
    ADD_INPUT(4, translation, DT_FLOAT, translationShape, 0.0);

    add1.set_attr_kernel_type("lanczos3");
    add1.set_attr_antialias(true);
