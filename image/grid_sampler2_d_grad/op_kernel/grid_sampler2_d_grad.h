    ...
    LocalTensor<T> dataTensor, LocalTensor<T> dupTensor, const T size, const int32_t calCount)
{
    ...
    if (padding == 1) {
        Compare(mask3Tensor, dataTensor, dataTensor, CMPMODE::EQ, newCalCount);
        PipeBarrier<PIPE_V>();
        ...
        PipeBarrier<PIPE_V>();
        Compare(mask3Tensor, tmpDataTensor, tmpDataTensor, CMPMODE::EQ, newCalCount);
        PipeBarrier<PIPE_V>();
...
template <typename T, typename GridSamplerGradTilingData>
__aicore__ inline void GridSampler2DGrad<T, GridSamplerGradTilingData>::ReflectCoordinatesCommon(
    LocalTensor<T> dataTensor, LocalTensor<T> dupTensor, const T size, int32_t newCalCount)
{
...
template <typename T, typename GridSamplerGradTilingData>
__aicore__ inline void GridSampler2DGrad<T, GridSamplerGradTilingData>::ClipCoordinatesSetGrad(LocalTensor<T> dataTensor,
...
template <typename T, typename GridSamplerGradTilingData>
__aicore__ inline void GridSampler2DGrad<T, GridSamplerGradTilingData>::ReflectCoordinatesSetGrad(
    LocalTensor<T> dataTensor, LocalTensor<T> dupTensor, LocalTensor<T> tmpDataTensor, LocalTensor<T> tmpDupTensor,
    LocalTensor<int32_t> tmpIntTensor, LocalTensor<T> extraTensor, LocalTensor<T> flipTensor, int64_t twiceLow,
...
template <typename T, typename GridSamplerGradTilingData>
__aicore__ inline void GridSampler2DGrad<T, GridSamplerGradTilingData>::ComputeIndex(
    LocalTensor<int32_t> dstIndex, LocalTensor<int32_t> dstIndex2, LocalTensor<int32_t> yCoor,
    LocalTensor<int32_t> xCoor, const int32_t calCount)
{
    if (interpolation == 0) {
    ...
    const int32_t coorIndex, const int32_t batchIdx)
{
    ...
    bool inRangeY = y >= 0 && y < height;
    bool inRangeX = x >= 0 && x < width;
    ...
    const int32_t computeCount, const int64_t curGridPointIndex)
{
    ...
    if (interpolation == 0) {
        ...
        for (int32_t i = 0; i < computeCount / 2; i++) {
            ...
            ComputeAfterTransposeGridGrad(nwIndex, iySe, yTensor, ixSe, xTensor, gOutLocalTensor, iyNw, ixNw, i, n);
            ComputeAfterTransposeXGrad(nwIndex2, nw, i, ncBaseOffset, gOutLocalTensor, iyNw, ixNw);
            ComputeAfterTransposeGridGrad(neIndex, yTensor, iySw, xTensor, ixSw, gOutLocalTensor, iyNe, ixNe, i, n);
            ComputeAfterTransposeXGrad(neIndex2, ne, i, ncBaseOffset, gOutLocalTensor, iyNe, ixNe);
            ComputeAfterTransposeGridGrad(swIndex, yTensor, iyNe, xTensor, ixNe, gOutLocalTensor, iySw, ixSw, i, n);
            ComputeAfterTransposeXGrad(swIndex2, sw, i, ncBaseOffset, gOutLocalTensor, iySw, ixSw);
            ComputeAfterTransposeGridGrad(seIndex, iyNw, yTensor, ixNw, xTensor, gOutLocalTensor, iySe, ixSe, i, n);
            ComputeAfterTransposeXGrad(seIndex2, se, i, ncBaseOffset, gOutLocalTensor, iySe, ixSe);
    ...
    if (interpolation == 1) {
        ...
        if (channel <= CHANNEL_1024) {
            ...
            for (int32_t i = 0; i < computeCount / 2; i++) {
                ...
                SetFlag<HardEvent::MTE2_S>(eventID);
                WaitFlag<HardEvent::MTE2_S>(eventID);
                ComputeNearestXGrad(xIndex, dupOneTensor, i, 0, ncBaseOffset, gOutLocalTensor, iyFloat, ixFloat);
                dataInQueue[0].FreeTensor(gOutLocalTensor);
            }
        }
