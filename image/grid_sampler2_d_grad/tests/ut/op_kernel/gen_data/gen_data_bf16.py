...
def write_file(shape, input):
    if "x" in input:
        x = np.random.randint(1, 255, shape).astype(tf.bfloat16.as_numpy_dtype)
        x.tofile("./x_16.bin")
    if "grid" in input:
        grid = np.random.random(shape).astype(tf.bfloat16.as_numpy_dtype)
        grid.tofile("./grid_16.bin")
    if "grad" in input:
        grad = np.random.random(shape).astype(tf.bfloat16.as_numpy_dtype)
