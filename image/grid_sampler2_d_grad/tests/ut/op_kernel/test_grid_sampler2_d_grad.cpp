...
protected:
    static void SetUpTestCase() { std::cout << "grid_sampler_2d_grad SetUp\n" << std::endl; }
    static void TearDownTestCase() { std::cout << "grid_sampler_2d_grad TearDown\n" << std::endl; }
};
...
TEST_F(grid_sampler_2d_grad_test, test_float_case)
{
    ...
    ICPU_RUN_KF(grid_sampler2_d_grad, numBlocks, grad, x, grid, dx, dgrid, workspace, tiling);
...
TEST_F(grid_sampler_2d_grad_test, test_float_case2)
{
    uint32_t N = 8;
    uint32_t C = 8;
    uint32_t H = 8;
...
TEST_F(grid_sampler_2d_grad_test, test_float16_case1)
{
    uint32_t N = 8;
    uint32_t C = 8;
    uint32_t H = 8;
    ...
    size_t dxByteSize = N * C * H * W * sizeof(float);
    size_t dgridByteSize = N * 2 * gH * gW * sizeof(float);
    Tiling4GridSampler2DGradCompileInfo compileInfo = {1, 196608};
    gert::TilingContextPara tilingContextPara(
        "GridSampler2DGrad",
...
TEST_F(grid_sampler_2d_grad_test, test_float16_case2)
{
    uint32_t N = 8;
    uint32_t C = 8;
    uint32_t H = 8;
...
TEST_F(grid_sampler_2d_grad_test, test_bfloat16_case1)
{
    uint32_t N = 8;
    uint32_t C = 8;
    uint32_t H = 8;
...
TEST_F(grid_sampler_2d_grad_test, test_bfloat16_case2)
{
    uint32_t N = 8;
    uint32_t C = 8;
    uint32_t H = 8;
...
//                                                 Ops::Cv::AnyValue::CreateFrom<std::string>("nearest")),
