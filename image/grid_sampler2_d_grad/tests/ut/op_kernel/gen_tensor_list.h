 ...
 */
#ifndef GEN_TENSOR_LIST_H
#define GEN_TENSOR_LIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* BinaryCreateTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, int ij)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        uint8_t* dataPtr = (uint8_t*)AscendC::GmAlloc(CeilA2B(dataSize, 32) * 32 + 64);
        dataPtr = (uint8_t*)((((uint64_t)dataPtr + 32 - 1) / 32) * 32);
        std::stringstream fileName;
        fileName << "./gen_data/input" << ij << "_t" << i << ".bin";
...
template <typename T>
void BinaryFreeTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./gen_data/output_t" << i << ".bin";
