...
__aicore__ inline void BarycentricFromImgcoordAIV<T>::PreProcess(uint32_t loopId, uint32_t curCalPixNum)
{
    ...
    for (uint32_t preLoopId = 0; preLoopId < preLoopNum; preLoopId++) {
        ...
        ReduceMinDepth(loopId, preLoopId, curPrePixNum, pingFlag);

        // ReduceMinDepth completes its scalar-to-MTE2 synchronization internally.
        pingFlag = 1 - pingFlag;
    }
    PipeBarrier<PIPE_ALL>();
}
...
template <typename T>
__aicore__ inline void BarycentricFromImgcoordAIV<T>::ReduceMinDepth(uint32_t loopId, uint32_t preLoopId,
                                                                     uint32_t curPrePixNum, uint32_t pingFlag)
{
    LocalTensor<float>
    ...
    // WholeReduceMin only defines the minimum-depth lane. Golden processes faces in
    // ascending order and uses a strict depth comparison, so equal-depth candidates
    // must deterministically keep the smaller face index.
    SetFlag<HardEvent::V_S>(EVENT_ID0);
    WaitFlag<HardEvent::V_S>(EVENT_ID0);

    LocalTensor<uint32_t> minCoreLocal = dstLocal.template ReinterpretCast<uint32_t>();
    uint64_t globalPixOffset = static_cast<uint64_t>(this->startPixId) + loopId * MAX_PROC_ELENUM +
                               preLoopId * TRANS_COL_ELENUM;
    for (uint32_t pixId = 0; pixId < curPrePixNum; ++pixId) {
        uint32_t bestCore = minCoreLocal.GetValue(pixId);
        float bestDepth = srcLocal.GetValue(pixId * ELENUM_REPEAT_FP32 + bestCore);
        uint64_t bestFaceOffset = static_cast<uint64_t>(bestCore) * this->totalPixNum + globalPixOffset + pixId;
        int32_t bestFace = this->zBufIdxGM.GetValue(bestFaceOffset);

        for (uint32_t coreIdx = 0; coreIdx < this->vecCoreNum; ++coreIdx) {
            float candidateDepth = srcLocal.GetValue(pixId * ELENUM_REPEAT_FP32 + coreIdx);
            if (candidateDepth != bestDepth) {
                continue;
            }
            uint64_t candidateFaceOffset = static_cast<uint64_t>(coreIdx) * this->totalPixNum + globalPixOffset + pixId;
            int32_t candidateFace = this->zBufIdxGM.GetValue(candidateFaceOffset);
            if (candidateFace < bestFace) {
                bestFace = candidateFace;
                bestCore = coreIdx;
            }
        }
        minCoreLocal.SetValue(pixId, bestCore);
    }
...
__aicore__ inline void BarycentricFromImgcoordAIV<T>::GenScreenVertData(uint32_t loopId, uint32_t curCalPixNum)
{
    ...
    uint32_t firstPixelId = this->startPixId + loopId * MAX_PROC_ELENUM;
    uint32_t imageWidth = static_cast<uint32_t>(this->width);

    // Pixel coordinates are integer quantities. Computing row with a rounded FP32
    // reciprocal maps boundary pixels (for example width 97/47/61) to the wrong row.
    // Initialize padding lanes as well because the following vector operations use
    // the repeat-aligned count.
    for (uint32_t i = 0; i < static_cast<uint32_t>(count); ++i) {
        float pixelX = SCREEN_PIX_COEF;
        float pixelY = SCREEN_PIX_COEF;
        if (i < curCalPixNum) {
            uint32_t pixelId = firstPixelId + i;
            uint32_t row = pixelId / imageWidth;
            uint32_t col = pixelId % imageWidth;
            pixelX = static_cast<float>(col) + SCREEN_PIX_COEF;
            pixelY = static_cast<float>(row) + SCREEN_PIX_COEF;
        }
        this->screenVertxLocal.SetValue(i, pixelX);
        this->screenVertyLocal.SetValue(i, pixelY);
    }
...
__aicore__ inline void BarycentricFromImgcoordAIV<T>::GenBarycentricCoord(uint32_t curCalPixNum)
{
    ...
    LocalTensor<uint8_t> sumNonZeroMaskLocal = this->screenVertxLocal.template ReinterpretCast<uint8_t>();
    int32_t count = ((curCalPixNum - 1) / ELENUM_REPEAT_FP32 + 1) * ELENUM_REPEAT_FP32;
    ...
    // Perspective correction is completed before applying the zero-area
    // sentinel, matching Golden's final torch.where(area != 0, corrected, -1).
    ...
    // Golden uses safe_sum = 1 only when corrected_sum is exactly zero.
    CompareScalar(sumNonZeroMaskLocal, wCoefLocal, 0.0f, CMPMODE::NE, count);
    PipeBarrier<PIPE_V>();
    Select(wCoefLocal, sumNonZeroMaskLocal, wCoefLocal, 1.0f, SELMODE::VSEL_TENSOR_SCALAR_MODE, count);
    PipeBarrier<PIPE_V>();
    ...
    PipeBarrier<PIPE_ALL>(); // both masks can be reused after the final select
...
{
    GenMaskDataInGM();
    // maskGM is produced cooperatively by all AIV cores. Ensure the whole mask
    // is visible before any active core copies it back for Gather.
    AscendC::SyncAll();
