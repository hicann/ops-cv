...
@register("upsample_bilinear_grad")
class FunctionTorchUpsampleBilinear2dGradApi(FunctionApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        if self.device == "pyaclnn":
            # intputSize_int输入是正向的input.shape
            ...
            input_data.kwargs["outputSize_int"][0] = self.outshape[-2]
            input_data.kwargs["outputSize_int"][1] = self.outshape[-1]

            if input_data.kwargs["align_corners"] == True:
                if input_data.kwargs["outputSize_int"][0] == 1:
                    input_data.kwargs["scalesH_double"] = 0
                if input_data.kwargs["outputSize_int"][1] == 1:
                    input_data.kwargs["scalesW_double"] = 0
                if input_data.kwargs["outputSize_int"][0] != 1 and input_data.kwargs["outputSize_int"][1] != 1:
                    input_data.kwargs["scalesH_double"] = (input_data.kwargs["intputSize_int"][2] - 1)/(input_data.kwargs["outputSize_int"][0] - 1)
                    input_data.kwargs["scalesW_double"] = (input_data.kwargs["intputSize_int"][3] - 1)/(input_data.kwargs["outputSize_int"][1] - 1)
                self.outshape[-1] = math.floor(input_data.kwargs["scalesW_double"])
                self.outshape[-2] = math.floor(input_data.kwargs["scalesH_double"])
            if input_data.kwargs["align_corners"] == False:
                if input_data.kwargs["scalesH_double"] <= 0 or input_data.kwargs["scalesW_double"] <= 0:
                    input_data.kwargs["scalesH_double"] = (input_data.kwargs["intputSize_int"][2])/(input_data.kwargs["outputSize_int"][0])
                    input_data.kwargs["scalesW_double"] = (input_data.kwargs["intputSize_int"][3])/(input_data.kwargs["outputSize_int"][1])
                else:
                    input_data.kwargs["outputSize_int"][0] = math.floor(input_data.kwargs["intputSize_int"][2] * input_data.kwargs["scalesH_double"])
                    input_data.kwargs["outputSize_int"][1] = math.floor(input_data.kwargs["intputSize_int"][3] * input_data.kwargs["scalesW_double"])
            ...
            #处理HW的取值,通过scale_factor和size取值计算outputSize_int的取值，区分是tuple和非tuple
            if 'scale_factor' in input_data.kwargs and isinstance(input_data.kwargs['scale_factor'], tuple):
                ...
                input_data.kwargs['outputSize_int'][0] = int(
                    input_data.kwargs['input'].shape[2]*input_data.kwargs['scale_factor'])
                input_data.kwargs['outputSize_int'][1] = int(
                    input_data.kwargs['input'].shape[3]*input_data.kwargs['scale_factor'])
                del input_data.kwargs['scale_factor']
