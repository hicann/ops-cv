...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    crop1.set_input_##intputName(placeholder##intputIndex);                                                         \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    crop1.set_input_##intputName(placeholder##intputIndex);                                                         \
    crop1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                          \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    crop1.update_output_desc_##outputName(outputName##outputIndex##_desc);
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto crop1 = op::Crop("crop1");
    // x: (4, 4, 8, 8) float32, size: (4, 4, 5, 5) float32, axis=2, offsets=[1, 2]
    // output y: (4, 4, 5, 5) float32
    std::vector<int64_t> xShape = {4, 4, 8, 8};
    std::vector<int64_t> sizeShape = {4, 4, 5, 5};
    std::vector<int64_t> yShape = {4, 4, 5, 5};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, size, inDtype, sizeShape);

    // 设置属性: axis=2, offsets=[1, 2]
    crop1.set_attr_axis(static_cast<int64_t>(2));
    std::vector<int64_t> offsetsAttr = {1, 2};
    crop1.set_attr_offsets(offsetsAttr);

    ADD_OUTPUT(1, y, inDtype, yShape);

    outputs.push_back(crop1);
    // 添加完毕
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> global_options = {{"ge.exec.deviceId", "5"}, {"ge.graphRunMode", "1"}};
