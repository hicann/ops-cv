...
@register("upsample_bicubic2d_grad")
class FunctionTorchUpsampleBicubic2dGradApi(FunctionApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        origin_dtype = input_tensor.dtype
        
        if self.device == "cpu":
            args =self.change_data_dtype(input_data.args,origin_dtype,torch.float32)
            kwargs = self.change_data_dtype(input_data.kwargs,origin_dtype,torch.float32)
        else:
        ...
        #使用gradOutput = torch.randn(output.shape, dtype=output.dtype)不能保证随机种子一致，使用torch.ones测试验证
        gradoutput = torch.ones_like(output).to(output.dtype)
        output.backward(gradoutput)
        backmarkgradoutput = kwargs['input'].grad.to(origin_dtype)
