...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    pasteSubImg1.set_input_##intputName(placeholder##intputIndex);                                                  \
    inputs.push_back(placeholder##intputIndex);

#define ADD_CONST_INT32_INPUT(intputIndex, intputName, inputShape, coordValues)                                     \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                                                  \
    ...
    placeholder##intputIndex##_desc.SetFormat(FORMAT_ND);                                                           \
    Tensor tensor_placeholder##intputIndex;                                                                         \
    ret = GenInt32Data(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                           \
                       placeholder##intputIndex##_desc, coordValues);                                               \
    if (ret != SUCCESS) {                                                                                           \
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());                              \
        return FAILED;                                                                                              \
    }                                                                                                               \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    pasteSubImg1.set_input_##intputName(placeholder##intputIndex);                                                  \
    pasteSubImg1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                   \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    pasteSubImg1.update_output_desc_##outputName(outputName##outputIndex##_desc);
                    ...
                    int value)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        uint32_t offset = i * GetDataTypeSize(data_type);
        if (data_type == ge::DT_FLOAT || data_type == ge::DT_FLOAT16 || data_type == ge::DT_BF16) {
            float fval = static_cast<float>(value);
            memcpy(pData + offset, &fval, GetDataTypeSize(data_type));
        } else {
            memcpy(pData + offset, &value, GetDataTypeSize(data_type));
        }
    }
...
{
    Status ret = SUCCESS;
    auto pasteSubImg1 = op::PasteSubImg("pasteSubImg1");
    // patch_img: (4, 4, 3) float32
    // patch_coord (offsets): [px1=1, py1=1, px2=0, py2=0] int32
    // core_area_coord (indices): [cx1=0, cy1=0, cx2=2, cy2=2] int32
    // combine_img: (4, 4, 3) float32
    // scale: 1.0
    // output combine_img: (4, 4, 3) float32
    std::vector<int64_t> patchImgShape = {4, 4, 3};
    std::vector<int64_t> combineImgShape = {4, 4, 3};
    std::vector<int64_t> coordShape = {4};

    ADD_INPUT(1, patch_img, inDtype, patchImgShape);
    ADD_CONST_INT32_INPUT(2, patch_coord, coordShape, (std::vector<int32_t>{1, 1, 0, 0}));
    ADD_CONST_INT32_INPUT(3, core_area_coord, coordShape, (std::vector<int32_t>{0, 0, 2, 2}));
    ADD_INPUT(4, combine_img, inDtype, combineImgShape);

    pasteSubImg1.set_attr_scale(1.0f);

    ADD_OUTPUT(1, combine_img, inDtype, combineImgShape);

    outputs.push_back(pasteSubImg1);
    return SUCCESS;
}
