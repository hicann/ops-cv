...
- API description: Applies the nearest neighbor interpolation algorithm to upsample the input signal composed of several input channels. If the input shape is **(N, C, H, W)**, the output shape is **(N, C, outputSize[0], outputSize[1])**. Compared with [aclnnUpsampleNearest2d](../../resize_nearest_neighbor_v2/docs/aclnnUpsampleNearest2d_en.md), this API adds the input parameters **scalesH** and **scalesW**. You should select a proper API based on the actual situation.
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnUpsampleNearest2dV2GetWorkspaceSize** is called to obtain the workspace size required for computation and the executor that contains the operator computation process. Then, **aclnnUpsampleNearest2dV2** is called to perform computation.
...
- **Parameters:**
  ...
  <tbody>
    <tr>
      ...
      <td><ul><li>Empty tensors are supported. </li><li>If the data type is DOUBLE or UINT8, the H and W axes of self must be less than 2^24. </li><li>For other data types, the values of self in all dimensions must be less than or equal to (2^31 – 1).</li></ul></td>
    ...
    <tr>
      ...
      <td><ul><li>Empty tensors are supported. </li><li>The data type and format must be the same as those of self. </li><li>If the data type is FLOAT32, BFLOAT16, or FLOAT16, the values of out in all dimensions are less than or equal to (2^31 – 1).</li></ul></td>
      <td>FLOAT32, BFLOAT16, FLOAT16, DOUBLE, UINT8</td>
      <td>NCHW and NHWC</td>
      <td>4</td>
      <td>√</td>
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnUpsampleNearest2dV2GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Either **outputSize** or **scalesH**/**scalesW** must be used.
  ...
  - If the values of **scalesH** and **scalesW** are greater than 0, the values of **scalesH** and **scalesW** are used, $outputSize=[floor(self\_H * scalesH), and floor(self\_W * scalesW)]$.
...
int main()
{
    ...
    aclOpExecutor *executor;
    // Call the first-phase API of aclnnUpsampleNearest2dV2.
    ...
    // Call the second-phase API of aclnnUpsampleNearest2dV2.
