...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    y_golden = interpolate(x_tensor, output_size, mode='nearest')
    tmp_golden = np.array(y_golden).astype(np_type)

    tmp_input.astype(np_type).tofile(f"{d_type}_input_upsample_nearest3d.bin")
    tmp_golden.astype(np_type).tofile(f"{d_type}_golden_upsample_nearest3d.bin")
