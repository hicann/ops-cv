...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    x_tensor = torch.tensor(tmp_input.astype(np.float32), dtype=torch.float32).requires_grad_(True)
    y_tensor = interpolate(x_tensor, output_size, mode='bilinear', align_corners=False, antialias=True)
    ...
    tmp_grad_output.tofile(f"{d_type}_input_upsample_bilinear2d_aa_backward.bin")
    tmp_golden.tofile(f"{d_type}_golden_upsample_bilinear2d_aa_backward.bin")
