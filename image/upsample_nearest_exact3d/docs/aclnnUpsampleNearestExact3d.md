...
- 计算公式：
  ...
  outputSize[2]/W &  Otherwise\\
  \end{cases}
  $$

  $$
  d_{src} = min(floor((d_{dst} + 0.5) / scaleD),  D - 1)
  $$
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnUpsampleNearestExact3dGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnUpsampleNearestExact3d”接口执行计算。
...
- 当输入scalesD、scalesH、scalesW的取值均大于0时，参数self、outputSize、scalesD、scalesH、scalesW需要满足如下约束：
...
int main()
{
    ...
    ret = aclnnUpsampleNearestExact3dGetWorkspaceSize(self, outputSize, scalesD, scalesH, scalesW, out, &workspaceSize,
                                                      &executor);
