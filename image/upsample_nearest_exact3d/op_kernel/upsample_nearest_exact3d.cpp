...
#include "../upsample_nearest3d/upsample_nearest3d_struct.h"
#else
...
__global__ __aicore__ void upsample_nearest_exact3d(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    UpsampleNearest3d310pKernelImpl<D_T_X, D_T_Y>(x, y, true, userWS, &tilingData);
#else
    REGISTER_TILING_DEFAULT(UpsampleNearest3dTilingData);
    GET_TILING_DATA_WITH_STRUCT(UpsampleNearest3dTilingData, tilingData, tiling);
    UpsampleNearest3dKernelImpl<D_T_X, D_T_Y>(x, y, true, &tilingData);
#endif
}
