...
#include "nn_other.h"
#include "../../op_graph/extract_image_patches_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                   \
    ...
    placeholder##intputIndex##_desc.SetOriginFormat(FORMAT_NHWC);                                                     \
...
int32_t GenOnesDataFloat32(vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, float value)
{
    ...
    if (pData == nullptr) {
        printf("%s - ERROR - [XIR]: Allocate memory for input data failed\n", GetTime().c_str());
        return FAILED;
    }
...
{
    Status ret = SUCCESS;
    auto add1 = op::ExtractImagePatches("extract1");
    add1.set_attr_ksizes({1, 2, 2, 1});
    add1.set_attr_strides({1, 1, 1, 1});
    add1.set_attr_rates({1, 1, 1, 1});
    add1.set_attr_padding("VALID");

    std::vector<int64_t> xShape = {1, 3, 3, 1};
    ADD_INPUT(1, x, inDtype, xShape);

    std::vector<int64_t> yShape = {1, 2, 2, 4};
...
int main(int argc, char* argv[])
{
    ...
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Session add ir compute graph to ir session failed\n", GetTime().c_str());
        delete session;
        GEFinalize();
        return FAILED;
    }
    ...
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Dump graph to txt failed\n", GetTime().c_str());
        delete session;
        GEFinalize();
        return FAILED;
    }
