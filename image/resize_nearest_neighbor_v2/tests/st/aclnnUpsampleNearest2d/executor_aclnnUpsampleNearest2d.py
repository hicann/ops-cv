...
@register("upsample_nearest_2d")
class FunctionTorchUpsampleNearest2dApi(FunctionApi):
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        origin_dtype = input_data.kwargs["input"].dtype
        x = input_data.kwargs["input"]
        size = input_data.kwargs["size"]
        if self.device == 'cpu' and (x.dtype == torch.float16 or x.dtype == torch.bfloat16):
            output = torch.nn.functional.interpolate(x.to(torch.float32), size=size, mode='nearest').to(origin_dtype)
        else:
            output = torch.nn.functional.interpolate(x, size=size, mode='nearest')
        return output
