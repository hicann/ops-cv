...
template <typename T, int cutNH>
__aicore__ inline void ResizeNearestNeighborV2NHWC<T, cutNH>::Init(GM_ADDR x, GM_ADDR size, GM_ADDR y,
                                                                   const ResizeNearestNeighborV2TilingData* tilingData)
{
...
template <typename T, int cutNH>
__aicore__ inline void ResizeNearestNeighborV2NHWC<T, cutNH>::DataCopyIn(int64_t n, int64_t hiStart, int64_t hiSize,
...
template <typename T, int cutNH>
__aicore__ inline void ResizeNearestNeighborV2NHWC<T, cutNH>::DataCopyOutNorH(LocalTensor<T>& xLocal, int64_t oncehSize,
                                                                              int64_t hiStart, int64_t n)
{
...
template <typename T, int cutNH>
__aicore__ inline void ResizeNearestNeighborV2NHWC<T, cutNH>::DataCopyOutNH(LocalTensor<T>& xLocal, int64_t oncehSize,
                                                                            int64_t idxStart)
{
    for (int32_t h = 0; h < oncehSize; h++) {
        LocalTensor<T> yLocal = outQue_.AllocTensor<T>();
...
template <typename T, int cutNH>
__aicore__ inline void ResizeNearestNeighborV2NHWC<T, cutNH>::ComputeDataCopyUb(int64_t n, int64_t hLoopTimes,
                                                                                int64_t hTail)
{
    ...
    if constexpr (cutNH == 2) {
        ...
        if (blockIdx_ < realCoreNum_ - 1) {
            for (int64_t n = 0; n < nLoopTimesBefore_; n++) { // 前面的核处理多少个n，h分核时直接等于N
                ComputeDataCopyUb(
                    n, hLoopTimesBB_,
                    hLoopTimesBT_); // h做ub切分，hLoopTimesBB_是h方向循环次数，hLoopTimesBT_是尾快处理多少个h
            }
        } else {
