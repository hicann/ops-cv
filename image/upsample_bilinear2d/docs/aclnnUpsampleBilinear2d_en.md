...
- Formulas:
  - Core algorithm logic:
    1. Each point of the target image is mapped back to the original image to obtain a coordinate with a decimal point.
    2. Based on the floating-point coordinate, the adjacent points of the original image are calculated.
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>Empty tensors are not supported. </li><li>The data type and format must be the same as those of self. </li><li>When its data type is DOUBLE, only the NHWC format is supported. </li><li>The shape supports only four dimensions, and the N and C axes must be the same as those of the input self shape.</li></ul></td>
      <td>FLOAT32, BFLOAT16, FLOAT16, DOUBLE</td>
      <td>NCHW and NHWC</td>
      <td>4</td>
      <td>√</td>
