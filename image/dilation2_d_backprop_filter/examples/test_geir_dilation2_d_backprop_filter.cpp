...
#include "../op_graph/dilation2_d_backprop_filter_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape, inputValue)                                     \
    ...
    ret = GenOnesData(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                            \
                      placeholder##intputIndex##_desc, intputDtype, inputValue);                                    \
    if (ret != SUCCESS) {                                                                                           \
...
int32_t WriteDataToFile(string bin_file, uint64_t data_size, uint8_t* inputData)
{
    ...
    if (fp == nullptr) {
        printf("%s - ERROR - [XIR]: Failed to open file %s for writing\n", GetTime().c_str(), bin_file.c_str());
        return FAILED;
    }
...
{
    Status ret = SUCCESS;
    auto add1 = op::Dilation2DBackpropFilter("add1");
    // x: shape (1, 3, 3, 1), NHWC
    std::vector<int64_t> xShape = {1, 3, 3, 1};
    ADD_INPUT(1, x, inDtype, xShape, 2);
    // filter: shape (2, 1, 1)
    std::vector<int64_t> filterShape = {2, 1, 1};
    ADD_INPUT(2, filter, inDtype, filterShape, 2);
    // out_backprop: shape (1, 2, 1, 1), NHWC
    std::vector<int64_t> outBpShape = {1, 2, 1, 1};
    ADD_INPUT(3, out_backprop, inDtype, outBpShape, 1);

    // y: shape (2, 1, 1), same as filter
    ADD_OUTPUT(1, y, inDtype, filterShape);

    add1.set_attr_strides({1, 1, 1, 1});
    add1.set_attr_rates({1, 1, 1, 1});
    add1.set_attr_padding_mode("VALID");
    add1.set_attr_pads({0, 0, 0, 0});
    add1.set_attr_ceil_mode(false);
