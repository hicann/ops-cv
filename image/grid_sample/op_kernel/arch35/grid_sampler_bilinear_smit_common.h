...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline float ClipCoordinates(float coord, T_IDX size)
{
...
template <typename T_IDX>
__simt_callee__ __aicore__ __attribute__((always_inline)) inline float reflectCoordinates(float coord, int twiceLow, T_IDX twiceHigh)
{
    ...
    T_IDX flips = static_cast<T_IDX>(floorf(coord / span));
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline float Clip(
    float coord, T_IDX size, int32_t paddingMode, int32_t alignCorners)
{
