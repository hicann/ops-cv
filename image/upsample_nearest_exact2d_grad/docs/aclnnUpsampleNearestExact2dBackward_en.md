...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>Empty tensors are supported. </li><li>The data type and format must be the same as those of gradOutput. </li><li>The N and C axes of the shape must be the same as those of the input parameter gradOutput.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>NCHW and ND</td>
      <td>4</td>
      <td>√</td>
...
- **Returns:**
  ...
  <tbody>
    ...
    <tr>
      <td>The data formats of gradOutput and out do not meet the requirements.</td>
    </tr>
  </tbody></table>
...
- In the input data scaling scenario, the scaling factor must be less than or equal to 50. That is:
  ...
  $$
  outputSize_H/Height of the output shape (H) <= 50
  $$
  
  $$
  outputSize_W/Width of the output shape (W) <= 50
  $$
