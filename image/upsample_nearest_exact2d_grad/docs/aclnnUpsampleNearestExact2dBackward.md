...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnUpsampleNearestExact2dBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnUpsampleNearestExact2dBackward”接口执行计算。
...
- 确定性计算：
  - aclnnUpsampleNearestExact2dBackward默认确定性实现。
...
int main()
{
    ...
    ret = aclnnUpsampleNearestExact2dBackwardGetWorkspaceSize(input, outputSizeArray, inputSizeArray, scalesH, scalesW,
                                                              out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS,
              LOG_PRINT("aclnnUpsampleNearestExact2dBackwardGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
