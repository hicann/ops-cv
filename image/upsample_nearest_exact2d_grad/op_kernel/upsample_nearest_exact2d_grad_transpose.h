...
public:
    ...
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output, GM_ADDR workspace, TPipe* pipeIn,
...
private:
    ...
    __aicore__ inline void CopyInAndDoComputeBigToSmall(struct IdxInfo idxInfo, LocalTensor<T> srcLocal,
                                                        LocalTensor<T> dstLocal, LocalTensor<T> inLocal,
                                                        LocalTensor<float> castOutLocal);
    __aicore__ inline void CopyInAndDoComputeNotBigToSmall(struct IdxInfo idxInfo, LocalTensor<T> srcLocal,
                                                           LocalTensor<T> dstLocal, LocalTensor<T> inLocal,
                                                           LocalTensor<float> castOutLocal);
    __aicore__ inline void CopyOutNotBigToSmall(struct IdxInfo idxInfo, int64_t dstWLen, int64_t resizeW,
...
template <typename T, bool BigToSmall, bool WAlign>
__aicore__ inline void UpSampleNearestExact2dGradTranspose<T, BigToSmall, WAlign>::ComputeBigToSmall(int64_t hIdx,
                                                                                                     int64_t hDown,
                                                                                                     int64_t hUp)
{
    for (int64_t wIdx = startW; wIdx < endW; wIdx += slideSizeW) {
        ...
        while (batch < endBatches) {
            ...
            struct IdxInfo idxInfo = {hIdx, hDown, hUp, wIdx, srcStartW, srcEndW, batch, batchNum, srcGap, dstGap};
...
template <typename T, bool BigToSmall, bool WAlign>
__aicore__ inline void UpSampleNearestExact2dGradTranspose<T, BigToSmall, WAlign>::ComputeNotBigToSmall(int64_t hIdx,
                                                                                                        int64_t hDown,
                                                                                                        int64_t hUp)
{
    int64_t resizeW = 1;
    ...
    for (int64_t h = idxInfo.hDown; h < idxInfo.hUp; h++) {
        for (int64_t w = idxInfo.srcStartW; w < idxInfo.srcEndW; w++) {
            if (srcStride == 0) {
                ...
                DataCopy(srcLocal, inTensorsGM[inOffset],
    ...
    for (int64_t h = idxInfo.hDown; h < idxInfo.hUp; h++) {
        if (srcStride == 0) {
            for (int64_t n = idxInfo.batch; n < idxInfo.batch + idxInfo.batchNum; n++) {
                ...
                DataCopy(srcLocal[(n - idxInfo.batch) * srcWLen * C0], inTensorsGM[inOffset],
    ...
    if constexpr (WAlign) {
        if (dstStride == 0) {
            ...
            for (int64_t m = 0; m < srcWLen; m++) {
                ...
                DataCopy(outTensorsGM[outOffset], dstLocal[m * C0],
        ...
        for (int64_t m = 0; m < dstWLen; m++) {
            ...
            if (dstStride == 0) {
                for (int64_t n = idxInfo.batch; n < idxInfo.batch + idxInfo.batchNum; n++) {
                    ...
                    DataCopy(outTensorsGM[outOffset],
                             dstLocal[(n - idxInfo.batch) * srcWLen * C0 +
                                      (srcIndexLocal.GetValue(m) - idxInfo.srcStartW) * C0],
                             {1, blockLen, 0, 0});
                }
            } else {
                int64_t outOffset = (idxInfo.batch * oh * ow + idxInfo.hIdx * ow + idxInfo.wIdx + m) * C0;
                DataCopy(outTensorsGM[outOffset], dstLocal[(srcIndexLocal.GetValue(m) - idxInfo.srcStartW) * C0],
                         {idxInfo.batchNum, blockLen, static_cast<uint16_t>((srcWLen - 1) * blockLen),
                          static_cast<uint16_t>(idxInfo.dstGap)});
            }
        }
