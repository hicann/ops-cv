...
|:--|:--|:--|:--|:--|
| boxes | 输入 | 候选框坐标，shape为(batch, num_boxes, q, 4)，最后一维依次表示y1、x1、y2、x2。 | FLOAT16、FLOAT32 | ND |
| scores | 输入 | 候选框分数，shape为(batch, num_boxes, num_classes)。 | FLOAT16、FLOAT32 | ND |
...
| iou_threshold | 输入 | IoU抑制阈值，Ascend 950支持0和正数。 | FLOAT32 | ND |
...
| nmsed_boxes | 输出 | 筛选后的候选框。 | FLOAT16、FLOAT32 | ND |
| nmsed_scores | 输出 | 筛选后的候选框分数。 | FLOAT16、FLOAT32 | ND |
| nmsed_classes | 输出 | 筛选后的类别索引，以浮点数表示。 | FLOAT16、FLOAT32 | ND |
| valid_detections | 输出 | 每个batch中有效候选框的数量。 | INT32 | ND |

## 约束说明

- `boxes`、`scores`和前三个输出的数据类型必须一致；两个阈值始终为FLOAT32。
- `boxes`必须为4维张量，`scores`必须为3维张量，且二者的batch和num_boxes维必须一致。
- `boxes`最后一维必须为4，q维必须为1或num_classes。
- batch、num_boxes和num_classes必须大于0；num_classes不能超过200。
- num_boxes不能超过`INT32_MAX`；batch与num_classes的乘积不能超过`INT32_MAX`；boxes的元素总数不能超过`INT64_MAX`。
