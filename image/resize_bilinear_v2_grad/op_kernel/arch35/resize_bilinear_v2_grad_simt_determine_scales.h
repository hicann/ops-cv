...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline T_IDX CalcInputIdx(T_IDX lenC, T_IDX resizedHeight,
                                                                                    T_IDX resizedWidth, T_IDX N,
                                                                                    T_IDX C, T_IDX H, T_IDX W)
{
    ...
    float scaleW, T_IDX N, T_IDX C, T_IDX H, T_IDX W, T_IDX inYStart, float outYStart, T_IDX inXStart, float outXStart)
{
    ...
    while (inY < resizedHeight) {
        ...
        while (outX < (static_cast<float>(W) + 1.0f) && inX < resizedWidth) {
            ...
            float YFloorIndex = floorf(outYClamped);
            float YTempIndex = fminf(YFloorIndex, static_cast<float>((lenSrcH - 1)));
            float lepy[2];
            lepy[0] = yLerp;
    ...
    float scaleW, T_IDX N, T_IDX C, T_IDX H, T_IDX W, T_IDX inYStart, float outYStart, T_IDX inXStart, float outXStart)
{
    ...
    while (outY < (static_cast<float>(H) + 1.0f) && inY < resizedHeight) {
        ...
        while (inX < resizedWidth) {
            ...
            float XFloorIndex = floorf(outXClamped);
            float XTempIndex = fminf(XFloorIndex, static_cast<float>((lenSrcW - 1)));
            float lepx[2];
            lepx[0] = xLerp;
    ...
    T_IDX blkStartOffset, T_IDX blkProcessNum)
{
    ...
    for (T_IDX idx = static_cast<T_IDX>(threadIdx.x); idx < blkProcessNum; idx += static_cast<T_IDX>(blockDim.x)) {
        ...
        if ((H != 0 && H != (lenSrcH - 1)) && (W != 0 && W != (lenSrcW - 1))) {
            acc = CalcInternalPoint<T1, T2, T_IDX, format, halfPixel>(grads, lenC, lenSrcH, lenSrcW, resizedHeight,
                                                                      resizedWidth, scaleH, scaleW, N, C, H, W,
                                                                      inYStart, outYStart, inXStart, outXStart);
        } else if ((H != 0 && H != (lenSrcH - 1)) && (W == 0 || W == (lenSrcW - 1))) {
            acc = CalcWBoundaryPoint<T1, T2, T_IDX, format, halfPixel>(grads, lenC, lenSrcH, lenSrcW, resizedHeight,
                                                                       resizedWidth, scaleH, scaleW, N, C, H, W,
                                                                       inYStart, outYStart, inXStart, outXStart);
        } else if ((H == 0 || H == (lenSrcH - 1)) && (W != 0 && W != (lenSrcW - 1))) {
            acc = CalcHBoundaryPoint<T1, T2, T_IDX, format, halfPixel>(grads, lenC, lenSrcH, lenSrcW, resizedHeight,
                                                                       resizedWidth, scaleH, scaleW, N, C, H, W,
                                                                       inYStart, outYStart, inXStart, outXStart);
        } else {
            acc = CalcCornerPoint<T1, T2, T_IDX, format, halfPixel>(grads, lenC, lenSrcH, lenSrcW, resizedHeight,
                                                                    resizedWidth, scaleH, scaleW, N, C, H, W, inYStart,
...
__simt_vf__ LAUNCH_BOUND(THREAD_NUM) __aicore__
    void calleeInt32ScalesDetermine(float scaleW, float scaleH, float inverseScaleW, float inverseScaleH,
                                    T_IDX resizedHeight, T_IDX resizedWidth, T_IDX lenN, T_IDX lenC, T_IDX lenSrcH,
                                    T_IDX lenSrcW, T_IDX coreNum, T_IDX shiftN, T_IDX mN, T_IDX shiftC, T_IDX mC,
                                    T_IDX shiftH, T_IDX mH, T_IDX shiftW, T_IDX mW, __gm__ T1* grads, __gm__ T2* y,
                                    int32_t blockId, T_IDX blkStartOffset, T_IDX blkProcessNum)
{
...
__simt_vf__ LAUNCH_BOUND(THREAD_NUM_MIDDLE) __aicore__
    void calleeInt64ScalesDetermine(float scaleW, float scaleH, float inverseScaleW, float inverseScaleH,
                                    T_IDX resizedHeight, T_IDX resizedWidth, T_IDX lenN, T_IDX lenC, T_IDX lenSrcH,
                                    T_IDX lenSrcW, T_IDX coreNum, T_IDX shiftN, T_IDX mN, T_IDX shiftC, T_IDX mC,
                                    T_IDX shiftH, T_IDX mH, T_IDX shiftW, T_IDX mW, __gm__ T1* grads, __gm__ T2* y,
                                    int32_t blockId, T_IDX blkStartOffset, T_IDX blkProcessNum)
...
__aicore__ inline void ResizeBilinearV2GradSimtDetermineScales<T1, T2, halfPixel, T_IDX, format>::Process()
{
    ...
    if constexpr (sizeof(T_IDX) == sizeof(uint64_t)) {
        asc_vf_call<calleeInt64ScalesDetermine<T1, T2, halfPixel, T_IDX, format>>(
        ...
        asc_vf_call<calleeInt32ScalesDetermine<T1, T2, halfPixel, T_IDX, format>>(
