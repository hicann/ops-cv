                                                ...
                                                bool halfPixelCenters, aclTensor* out, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ResizeBilinearV2Grad, OP_INPUT(gradOut, image), OP_OUTPUT(out),
...
{
    auto out = executor->AllocTensor(image->GetStorageShape(), image->GetOriginalShape(), image->GetDataType(),
                                     image->GetStorageFormat(), image->GetOriginalFormat());
...
static aclTensor* ResizeBilinearV2GradAiCore(const aclTensor* grads, const aclTensor* originalImage,
                                             const bool alignCorners, const bool halfPixelCenters,
                                             const aclFloatArray* scales, aclTensor* y, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(ResizeBilinearV2Grad, OP_INPUT(grads, originalImage), OP_OUTPUT(y),
...
{
    auto y = executor->AllocTensor(originalImage->GetViewShape(), originalImage->GetDataType(),
                                   originalImage->GetViewFormat());
