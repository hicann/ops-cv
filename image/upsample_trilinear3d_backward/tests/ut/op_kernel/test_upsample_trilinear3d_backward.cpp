...
protected:
    ...
    {
        std::cout << "upsample_trilinear3d_backward_test SetUp\n" << std::endl;
...
TEST_F(upsample_trilinear3d_backward_test, test_case_float_1) {
    ...
    size_t outputByteSize = 1 * 1 * 4 * 4 * 4 * sizeof(float);
    ...
    ICPU_RUN_KF(upsample_trilinear3d_backward, tilingInfo.blockNum, x, y, workspace, tiling);

    fileName = "./upsample_trilinear3d_backward_data/float32_output_upsample_trilinear3d_backward.bin";
...
TEST_F(upsample_trilinear3d_backward_test, test_case_float16_2) {
    ...
    uint8_t* x = (uint8_t*)AscendC::GmAlloc(CeilAlign(inputByteSize, 32));
    ReadFile(fileName, inputByteSize, x, inputByteSize);
    size_t outputByteSize = 1 * 1 * 4 * 4 * 4 * sizeof(half);
    uint8_t* y = (uint8_t*)AscendC::GmAlloc(CeilAlign(outputByteSize, 32));
    ...
    fileName = "./upsample_trilinear3d_backward_data/float16_output_upsample_trilinear3d_backward.bin";
    WriteFile(fileName, y, outputByteSize);
...
TEST_F(upsample_trilinear3d_backward_test, test_case_bfloat16_3) {
    ...
    fileName = "./upsample_trilinear3d_backward_data/bfloat16_output_upsample_trilinear3d_backward.bin";
    WriteFile(fileName, y, outputByteSize);
