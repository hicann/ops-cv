...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void ComputeLimt(T2 inLStart, T2 lenDesL, T2 srcL1,
                                                                                  T2 NC, T2 L, float outLStart,
                                                                                  float scaleL, float& acc,
                                                                                  __gm__ T1* inputGm)
{
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void SimtComputeDetermine(
    T2 blkStartOffset, T2 blkProcessNum, T2 mL, T2 shiftL, T2 lenDesL, T2 lenSrcLOrUb, float scaleL,
    float inverseScaleL, __gm__ T1* inputGm, __gm__ T1* outputGm)
{
    ...
    for (T2 idx = static_cast<T2>(threadIdx.x); idx < blkProcessNum; idx += static_cast<T2>(blockDim.x)) {
        ...
        if constexpr (isCenter == 1) {
            inLStart = static_cast<T2>(
                max(static_cast<T3>(0), static_cast<T3>(ceilf((static_cast<float>(L) - 1.0f) * inverseScaleL))));
            outLStart = static_cast<float>(inLStart) * scaleL;
        } else {
            inLStart = static_cast<T2>(
                max(static_cast<T3>(0),
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void SimtComputeDetermineMode2(
    T2 blkStartOffset, T2 blkProcessNum, T2 lenDesL, __gm__ T1* inputGm, __gm__ T1* outputGm)
{
    for (T2 idx = static_cast<T2>(threadIdx.x); idx < blkProcessNum; idx += static_cast<T2>(blockDim.x)) {
...
__simt_vf__ LAUNCH_BOUND(512) __aicore__
    void calleeInt64Determine(T2 blkStartOffset, T2 blkProcessNum, T2 mL, T2 shiftL, T2 lenDesL, T2 lenSrcLOrUb,
                              float scaleL, float inverseScaleL, __gm__ T1* inputGm, __gm__ T1* outputGm)
{
...
__simt_vf__ LAUNCH_BOUND(1024) __aicore__
    void calleeInt32Determine(T2 blkStartOffset, T2 blkProcessNum, T2 mL, T2 shiftL, T2 lenDesL, T2 lenSrcLOrUb,
                              float scaleL, float inverseScaleL, __gm__ T1* inputGm, __gm__ T1* outputGm)
{
    SimtComputeDetermine<T1, T2, T3, isCenter>(blkStartOffset, blkProcessNum, mL, shiftL, lenDesL, lenSrcLOrUb, scaleL,
                                               inverseScaleL, inputGm, outputGm);
...
template <typename T1, typename T2, typename T3, uint64_t isCenter, uint64_t mode>
__aicore__ inline void ResizeLinearGradDetermine<T1, T2, T3, isCenter, mode>::Init(
...
__aicore__ inline void ResizeLinearGradDetermine<T1, T2, T3, isCenter, mode>::Process()
{
    ...
    if constexpr (mode == 2) {
        ...
        if constexpr (sizeof(T2) == sizeof(uint64_t)) {
            asc_vf_call<calleeInt64Determine<T1, T2, T3, isCenter>>(
                dim3(512), blkStartOffset, blkProcessNum, mL, shiftL, lenDesL, lenSrcLOrUb, scaleL, inverseScaleL,
                (__gm__ T1*)(inputGm_.GetPhyAddr()), (__gm__ T1*)(outputGm_.GetPhyAddr()));
        } else {
            asc_vf_call<calleeInt32Determine<T1, T2, T3, isCenter>>(
                dim3(1024), blkStartOffset, blkProcessNum, mL, shiftL, lenDesL, lenSrcLOrUb, scaleL, inverseScaleL,
                (__gm__ T1*)(inputGm_.GetPhyAddr()), (__gm__ T1*)(outputGm_.GetPhyAddr()));
        }
    }
}
