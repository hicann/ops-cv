...
void Col2imTiling::SetTilingData()
{
    useCoreNum_ = static_cast<int64_t>(
        Ops::Base::CeilDiv(totalLength_, Ops::Base::CeilDiv(totalLength_, totalCoreNum_)));
...
ge::graphStatus Col2imTiling::SetAttrParams()
{
    ...
    kernelSizeW_ = static_cast<int64_t>(
        (reinterpret_cast<const int64_t*>(kernelSizePtr->GetData()))[SECOND_IDX_IN_VECTOR]);
    OP_CHECK_IF(kernelSizeH_ <= 0,
                OP_LOGE(context_, "kernelSizeH must be greater than 0, but this is %ld.", kernelSizeH_),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(kernelSizeW_ <= 0,
    ...
    OP_CHECK_IF(dilationH_ <= 0, OP_LOGE(context_, "dilationH must be greater than 0, but this is %ld.", dilationH_),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(dilationW_ <= 0, OP_LOGE(context_, "dilationW must be greater than 0, but this is %ld.", dilationW_),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(strideH_ <= 0, OP_LOGE(context_, "strideH must be greater than 0, but this is %ld.", strideH_),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(strideW_ <= 0, OP_LOGE(context_, "strideW must be greater than 0, but this is %ld.", strideW_),
                return ge::GRAPH_FAILED);
...
ge::graphStatus Col2imTiling::InitTilingData()
{
    ...
    OP_CHECK_IF((memset_s(tilingData_, sizeof(Col2imRegBaseTilingData), 0, sizeof(Col2imRegBaseTilingData)) != EOK),
...
uint64_t Col2imTiling::GetTilingKey() { return tilingKey_; }
...
ge::graphStatus Col2imTiling::GetInputTensorInfo()
{
    ...
    OP_CHECK_IF(outputSizeLength != 2,
...
ge::graphStatus TilingPrepareForCol2im([[maybe_unused]] gert::TilingParseContext* context) { return ge::GRAPH_SUCCESS; }

IMPL_OP_OPTILING(Col2im).Tiling(TilingForCol2im).TilingParse<Col2imCompileInfo>(TilingPrepareForCol2im);
} // namespace optiling