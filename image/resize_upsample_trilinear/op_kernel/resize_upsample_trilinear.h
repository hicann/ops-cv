    ...
    const int32_t omega_metrix_h, float scale, const int32_t input_start_indx, const int32_t input_end_indx)
{
    ...
    for (size_t i = 0; i < omega_metrix_h; i++) {
        realIndex = AreaPixelComputeSourceIndex(scale, output_index);
        int32_t input_idx_0 = MIN(static_cast<int32_t>(realIndex), input_end_indx);
        float lambda0 = MIN(MAX(realIndex - (float)input_idx_0, static_cast<float>(0.0)), static_cast<float>(1.0));
...
__aicore__ inline float KernelUpsampleTrilinear<T>::AreaPixelComputeSourceIndex(float scale, int32_t dst_index)
{
    ...
    if (align_corners == 1) {
        ...
        float srcIdx = static_cast<float>(scale * ((float)dst_index + (float)0.5) - (float)0.5);
        result = (srcIdx < zero) ? float(0.) : srcIdx;
    }
    return result;
}
