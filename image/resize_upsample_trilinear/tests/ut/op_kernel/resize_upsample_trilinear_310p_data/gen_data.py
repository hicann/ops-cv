...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    x_tensor = x_tensor.permute(3, 4, 0, 1, 2)
    ...
    tmp_input.astype(np_type).tofile(f"{d_type}_input_trilinear.bin")
    tmp_golden.astype(np_type).tofile(f"{d_type}_golden_trilinear3d.bin")
